...
impl<F: Field + Ord + std::hash::Hash> ic13Chip<F> {
    ...
    pub fn assign(
        ...
        let mut distances: HashMap<F, u64> = HashMap::new(); // 存储节点到源点的距离
        let mut predecessors: HashMap<F, F> = HashMap::new(); // 存储每个节点的前驱节点
        let mut q: VecDeque<F> = VecDeque::new(); // BFS 队列
        ...
        if distances.contains_key(&person_id1_val) {
            distances.insert(person_id1_val, 0); // 源点距离为0
            q.push_back(person_id1_val); // 源点入队
            ...
            |mut region| {
                ...
                for (i, row) in person_table.iter().enumerate() {
                    ...
                    let (predecessor_val, predecessor_dist) = if row[0] == person_id1_val {
                        ...
                        // if i == 70 {
                        //     println!("pre:{:?},cur:{:?}",f_to_u64(&pred_id),f_to_u64(&row[0]));
                        // }
