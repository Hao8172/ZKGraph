...
impl<F: Field + Ord> Is1Chip<F> {
    ...
    pub fn configure(meta: &mut ConstraintSystem<F>) -> Is1CircuitConfig<F> {
        ...
        // shuffle：验证 person index 符合 person_check
        ...
        // 验证 result 符合 column 中在 row_start 和 row_end 中
    ...
    pub fn assign(
        ...
        // 获取该 Person 关联的 Place 在 person_to_place_csr.column 数组中的起始 ( actual_start_offset ) 和结束 ( actual_end_offset ) 偏移量
        ...
        if let Some(person_row) = found_person {
            ...
            for place_id in &expected_places {
                ...
                result_row.push(*place_id); // 添加place_id作为最后一列
        ...
        layouter.assign_region(
            ...
            |mut region| {
                ...
                for (i, row) in person_table.iter().enumerate() {
                    ...
                    // 将 person_id 复制到单独设定的 advice 列用于 IsZero 对比
                    region.assign_advice(
                        || format!("person id {}", i),
