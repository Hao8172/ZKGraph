...
/*
MATCH path = shortestPath((p)-[:KNOWS*1..5]->(friend)) // ERWEITERT AUF 5 HOPS
*/
...
pub struct Is1CircuitConfig<F: Field + Ord + std::hash::Hash> {
    ...
    // --- Hop 4 Additions START ---
    // --- Extended Distinct 3-Hop Friends (for alignment for Hop 4) ---
    ...
    // --- Hop 3 Alignment (for PKP edges to determine if source/target is a 3-hop friend) ---
    ...
    // --- Hop 5 Additions START ---
    // --- Extended Distinct 4-Hop Friends (for alignment for Hop 5) ---
    ...
    // --- Hop 4 Alignment (for PKP edges to determine if source/target is a 4-hop friend) ---
...
impl<F: Field + Ord + std::hash::Hash> Is1Chip<F> {
    ...
    pub fn configure(meta: &mut ConstraintSystem<F>) -> Is1CircuitConfig<F> {
        ...
        // --- Hop 4 Additions (Configuration) START ---

        // --- hop3_ext: Padded and sorted list of 3-hop friends for alignment lookups (for Hop 4) ---
        ...
        meta.shuffle(
            ...
            |meta| {
                let q_src_distinct_h3 = meta.query_selector(q_distinct_hop3_to);
                let q_dest_ext_internal = meta.query_selector(q_distinct_hop3_to_ext_internal);
                let val_from_distinct_h3 = meta.query_advice(distinct_hop3_to, Rotation::cur());
                let val_in_ext_internal = meta.query_advice(distinct_hop3_to_ext, Rotation::cur());
                let lhs =
                    [one.clone(), val_from_distinct_h3].map(|v| v * q_src_distinct_h3.clone());
                let rhs =
                    [one.clone(), val_in_ext_internal].map(|v| v * q_dest_ext_internal.clone());
                lhs.into_iter().zip(rhs).collect()
            },
        ...
        meta.create_gate("distinct_hop3_to_ext boundary check", |meta| {
            let q_boundary_active = meta.query_selector(q_distinct_hop3_to_ext_boundary);
            let current_val = meta.query_advice(distinct_hop3_to_ext, Rotation::cur());
            vec![
                q_boundary_active
                    * current_val.clone()
                    * (current_val - Expression::Constant(F::from(MAX_PERSON_ID))),
        ...
        meta.create_gate("distinct_hop3_to_ext selector logic", |meta| {
            let q_ext_row_active = meta.query_selector(q_distinct_hop3_to_ext);
            let q_internal_part_active = meta.query_selector(q_distinct_hop3_to_ext_internal);
            let q_boundary_part_active = meta.query_selector(q_distinct_hop3_to_ext_boundary);
            vec![q_ext_row_active * (q_internal_part_active + q_boundary_part_active - one.clone())]
        });
        ...
        // --- Alignment columns for pkp.FromId (ordered_pkp[0]) against distinct_hop3_to_ext ---
        ...
        meta.lookup_any(
            ...
            |meta| {
                let q_pkp_active = meta.query_selector(q_pkp);
                let q_ext_table_active = meta.query_selector(q_distinct_hop3_to_ext);
                let val_to_check = meta.query_advice(aligned_h3_pkp_personid, Rotation::cur());
                let val_in_ext_table = meta.query_advice(distinct_hop3_to_ext, Rotation::cur());
                let lhs = [one.clone(), val_to_check].map(|v| v * q_pkp_active.clone());
                let rhs = [one.clone(), val_in_ext_table].map(|v| v * q_ext_table_active.clone());
                lhs.into_iter().zip(rhs).collect()
            },
        ...
        meta.lookup_any(
            ...
            |meta| {
                let q_pkp_active = meta.query_selector(q_pkp);
                let q_ext_table_active = meta.query_selector(q_distinct_hop3_to_ext);
                let val_to_check = meta.query_advice(next_aligned_h3_pkp_personid, Rotation::cur());
                let val_in_ext_table = meta.query_advice(distinct_hop3_to_ext, Rotation::cur());
                let lhs = [one.clone(), val_to_check].map(|v| v * q_pkp_active.clone());
                let rhs = [one.clone(), val_in_ext_table].map(|v| v * q_ext_table_active.clone());
                lhs.into_iter().zip(rhs).collect()
        ...
        meta.lookup_any("aligned_h3_pkp_personid pair lookup", |meta| {
            let q_pkp_active = meta.query_selector(q_pkp);
            let q_pairs_table_active = meta.query_selector(q_distinct_hop3_to_ext_pairs_lookup);
            let aligned_val = meta.query_advice(aligned_h3_pkp_personid, Rotation::cur());
            let next_aligned_val = meta.query_advice(next_aligned_h3_pkp_personid, Rotation::cur());
            let pair_first =
                meta.query_advice(distinct_hop3_to_ext_pairs_lookup_table[0], Rotation::cur());
            let pair_second =
                meta.query_advice(distinct_hop3_to_ext_pairs_lookup_table[1], Rotation::cur());
            let lhs =
                [one.clone(), aligned_val, next_aligned_val].map(|v| v * q_pkp_active.clone());
            let rhs =
                [one.clone(), pair_first, pair_second].map(|v| v * q_pairs_table_active.clone());
        ...
        // --- Alignment columns for pkp.ToId (ordered_pkp[1]) against distinct_hop3_to_ext ---
        ...
        meta.lookup_any(
            ...
            |meta| {
                let q_pkp_active = meta.query_selector(q_pkp);
                let q_ext_table_active = meta.query_selector(q_distinct_hop3_to_ext);
                let val_to_check = meta.query_advice(aligned_h3_pkp_to_personid, Rotation::cur());
                let val_in_ext_table = meta.query_advice(distinct_hop3_to_ext, Rotation::cur());
                let lhs = [one.clone(), val_to_check].map(|v| v * q_pkp_active.clone());
                let rhs = [one.clone(), val_in_ext_table].map(|v| v * q_ext_table_active.clone());
                lhs.into_iter().zip(rhs).collect()
        ...
        meta.lookup_any("aligned_h3_pkp_to_personid pair lookup", |meta| {
            let q_pkp_active = meta.query_selector(q_pkp);
            let q_pairs_table_active = meta.query_selector(q_distinct_hop3_to_ext_pairs_lookup);
            let aligned_val = meta.query_advice(aligned_h3_pkp_to_personid, Rotation::cur());
            let next_aligned_val =
                meta.query_advice(next_aligned_h3_pkp_to_personid, Rotation::cur());
            let pair_first =
                meta.query_advice(distinct_hop3_to_ext_pairs_lookup_table[0], Rotation::cur());
        ...
        // --- Hop 4: Find friends of 3-hop friends ---
        ...
        meta.create_gate("init_h4_states", |meta| {
            let q_person_active = meta.query_selector(q_person);
            let dist4_node_assigned_val = meta.query_advice(dist4_node, Rotation::cur());
            let q_dist4_node_selector_val = meta.query_selector(q_dist4_node);
            vec![q_person_active.clone() * (dist4_node_assigned_val - q_dist4_node_selector_val)]
        });

        let q_hop4_to = meta.complex_selector();
        meta.create_gate("set_q_hop4_to_selector", |meta| {
            let q_pkp_active = meta.query_selector(q_pkp);
            let q_hop4_to_val = meta.query_selector(q_hop4_to);
            let pkp_from_is_h3_friend = meta.query_advice(h3_match_flag, Rotation::cur());
            let pkp_to_is_source = meta.query_advice(source_pkp_to_check, Rotation::cur());
            let pkp_to_is_h1_friend = meta.query_advice(h1_pkp_to_flag, Rotation::cur());
            let pkp_to_is_h2_friend = meta.query_advice(h2_pkp_to_flag, Rotation::cur());
            let pkp_to_is_h3_friend = meta.query_advice(h3_pkp_to_flag, Rotation::cur());

            let condition = pkp_from_is_h3_friend
                * (one.clone() - pkp_to_is_source)
                * (one.clone() - pkp_to_is_h1_friend)
                * (one.clone() - pkp_to_is_h2_friend)
                * (one.clone() - pkp_to_is_h3_friend);
            vec![q_pkp_active.clone() * (q_hop4_to_val - condition)]
        });
        ...
        meta.create_gate("distinct_hop4_to_order and uniqueness", |meta| {
            ...
            let check = meta.query_advice(distinct_hop4_to_check, Rotation::cur());
            let cur_lte_next = distinct_hop4_to_order.is_lt(meta, None);
            vec![q.clone() * (cur_lte_next - one.clone()), q.clone() * check]
        });

        meta.shuffle("mark 4-hop friends in person table", |meta| {
            let q_distinct_h4_active = meta.query_selector(q_distinct_hop4_to);
            let q_person_dist4_active = meta.query_selector(q_dist4_node);
            let friend_id_from_distinct_h4 = meta.query_advice(distinct_hop4_to, Rotation::cur());
            let person_id_from_person_table = meta.query_advice(person[0], Rotation::cur());
            let lhs = [one.clone(), person_id_from_person_table]
                .map(|v| v * q_person_dist4_active.clone());
            let rhs =
                [one.clone(), friend_id_from_distinct_h4].map(|v| v * q_distinct_h4_active.clone());
            lhs.into_iter().zip(rhs).collect()
        });
        ...
        meta.create_gate("calculate potential_friend_hop4", |meta| {
            ...
            let pot_h3 = meta.query_advice(potential_friend_hop3, Rotation::cur());
            let pot_h4 = meta.query_advice(potential_friend_hop4, Rotation::cur());
            vec![q_person_active.clone() * (pot_h4 - (d4.clone() + pot_h3.clone() - d4 * pot_h3))]
        });
        // --- Hop 4 Additions (Configuration) END ---

        // --- Hop 5 Additions (Configuration) START ---

        // --- hop4_ext: Padded and sorted list of 4-hop friends for alignment lookups (for Hop 5) ---
        ...
        let distinct_hop4_to_ext_order_config = LtEqGenericChip::configure(
            meta,
            |meta| meta.query_selector(q_distinct_hop4_to_ext_order),
        ...
        meta.shuffle(
            "distinct_hop4_to to distinct_hop4_to_ext internal part",
            |meta| {
                let q_src_distinct_h4 = meta.query_selector(q_distinct_hop4_to);
                let q_dest_ext_internal = meta.query_selector(q_distinct_hop4_to_ext_internal);
                let val_from_distinct_h4 = meta.query_advice(distinct_hop4_to, Rotation::cur());
                let val_in_ext_internal = meta.query_advice(distinct_hop4_to_ext, Rotation::cur());
                let lhs =
                    [one.clone(), val_from_distinct_h4].map(|v| v * q_src_distinct_h4.clone());
                let rhs =
                    [one.clone(), val_in_ext_internal].map(|v| v * q_dest_ext_internal.clone());
                lhs.into_iter().zip(rhs).collect()
            },
        ...
        meta.create_gate("distinct_hop4_to_ext boundary check", |meta| {
            let q_boundary_active = meta.query_selector(q_distinct_hop4_to_ext_boundary);
            let current_val = meta.query_advice(distinct_hop4_to_ext, Rotation::cur());
            vec![
                q_boundary_active
                    * current_val.clone()
                    * (current_val - Expression::Constant(F::from(MAX_PERSON_ID))),
        ...
        meta.create_gate("distinct_hop4_to_ext selector logic", |meta| {
            let q_ext_row_active = meta.query_selector(q_distinct_hop4_to_ext);
            let q_internal_part_active = meta.query_selector(q_distinct_hop4_to_ext_internal);
            let q_boundary_part_active = meta.query_selector(q_distinct_hop4_to_ext_boundary);
            vec![q_ext_row_active * (q_internal_part_active + q_boundary_part_active - one.clone())]
        });

        let mut distinct_hop4_to_ext_pairs_lookup_table = Vec::new();
        for _ in 0..2 {
            distinct_hop4_to_ext_pairs_lookup_table.push(meta.advice_column());
        }
        for i in 0..2 {
            meta.enable_equality(distinct_hop4_to_ext_pairs_lookup_table[i]);
        }
        let q_distinct_hop4_to_ext_pairs_lookup = meta.complex_selector();

        // --- Alignment columns for pkp.FromId (ordered_pkp[0]) against distinct_hop4_to_ext ---
        let aligned_h4_pkp_personid = meta.advice_column();
        let next_aligned_h4_pkp_personid = meta.advice_column();

        meta.lookup_any(
            "aligned_h4_pkp_personid is from distinct_hop4_to_ext",
            |meta| {
                let q_pkp_active = meta.query_selector(q_pkp);
                let q_ext_table_active = meta.query_selector(q_distinct_hop4_to_ext);
                let val_to_check = meta.query_advice(aligned_h4_pkp_personid, Rotation::cur());
                let val_in_ext_table = meta.query_advice(distinct_hop4_to_ext, Rotation::cur());
                let lhs = [one.clone(), val_to_check].map(|v| v * q_pkp_active.clone());
                let rhs = [one.clone(), val_in_ext_table].map(|v| v * q_ext_table_active.clone());
                lhs.into_iter().zip(rhs).collect()
            },
        ...
        meta.lookup_any(
            "next_aligned_h4_pkp_personid is from distinct_hop4_to_ext",
            |meta| {
                let q_pkp_active = meta.query_selector(q_pkp);
                let q_ext_table_active = meta.query_selector(q_distinct_hop4_to_ext);
                let val_to_check = meta.query_advice(next_aligned_h4_pkp_personid, Rotation::cur());
                let val_in_ext_table = meta.query_advice(distinct_hop4_to_ext, Rotation::cur());
                let lhs = [one.clone(), val_to_check].map(|v| v * q_pkp_active.clone());
                let rhs = [one.clone(), val_in_ext_table].map(|v| v * q_ext_table_active.clone());
                lhs.into_iter().zip(rhs).collect()
        ...
        let aligned_h4_pkp_personid_config = LtEqGenericChip::configure(
            meta,
            |meta| meta.query_selector(q_pkp),
            |meta| vec![meta.query_advice(aligned_h4_pkp_personid, Rotation::cur())],
            |meta| vec![meta.query_advice(ordered_pkp[0], Rotation::cur())],
        );
        meta.create_gate("aligned_h4_pkp_personid_config verify", |meta| {
            let q = meta.query_selector(q_pkp);
            vec![q * (aligned_h4_pkp_personid_config.is_lt(meta, None) - one.clone())]
        });

        let next_aligned_h4_pkp_personid_config = LtEqGenericChip::configure(
            meta,
            |meta| meta.query_selector(q_pkp),
            |meta| vec![meta.query_advice(ordered_pkp[0], Rotation::cur())],
            |meta| {
                vec![meta.query_advice(next_aligned_h4_pkp_personid, Rotation::cur()) - one.clone()]
            },
        );
        meta.create_gate("next_aligned_h4_pkp_personid_config verify", |meta| {
            let q = meta.query_selector(q_pkp);
            vec![q * (next_aligned_h4_pkp_personid_config.is_lt(meta, None) - one.clone())]
        });

        meta.lookup_any("aligned_h4_pkp_personid pair lookup", |meta| {
            let q_pkp_active = meta.query_selector(q_pkp);
            let q_pairs_table_active = meta.query_selector(q_distinct_hop4_to_ext_pairs_lookup);
            let aligned_val = meta.query_advice(aligned_h4_pkp_personid, Rotation::cur());
            let next_aligned_val = meta.query_advice(next_aligned_h4_pkp_personid, Rotation::cur());
            let pair_first =
                meta.query_advice(distinct_hop4_to_ext_pairs_lookup_table[0], Rotation::cur());
            let pair_second =
                meta.query_advice(distinct_hop4_to_ext_pairs_lookup_table[1], Rotation::cur());
            let lhs =
                [one.clone(), aligned_val, next_aligned_val].map(|v| v * q_pkp_active.clone());
            let rhs =
                [one.clone(), pair_first, pair_second].map(|v| v * q_pairs_table_active.clone());
        ...
        let h4_match_flag_config = IsZeroChip::configure(
            meta,
            |meta| meta.query_selector(q_pkp),
            |meta| {
                meta.query_advice(aligned_h4_pkp_personid, Rotation::cur())
                    - meta.query_advice(ordered_pkp[0], Rotation::cur())
            },
            iz_h4_match_flag,
            h4_match_flag,
        ...
        // --- Alignment columns for pkp.ToId (ordered_pkp[1]) against distinct_hop4_to_ext ---
        let aligned_h4_pkp_to_personid = meta.advice_column();
        let next_aligned_h4_pkp_to_personid = meta.advice_column();

        meta.lookup_any(
            "aligned_h4_pkp_to_personid is from distinct_hop4_to_ext",
            |meta| {
                let q_pkp_active = meta.query_selector(q_pkp);
                let q_ext_table_active = meta.query_selector(q_distinct_hop4_to_ext);
                let val_to_check = meta.query_advice(aligned_h4_pkp_to_personid, Rotation::cur());
                let val_in_ext_table = meta.query_advice(distinct_hop4_to_ext, Rotation::cur());
                let lhs = [one.clone(), val_to_check].map(|v| v * q_pkp_active.clone());
                let rhs = [one.clone(), val_in_ext_table].map(|v| v * q_ext_table_active.clone());
                lhs.into_iter().zip(rhs).collect()
        ...
        meta.lookup_any(
            "next_aligned_h4_pkp_to_personid is from distinct_hop4_to_ext",
            |meta| {
                let q_pkp_active = meta.query_selector(q_pkp);
                let q_ext_table_active = meta.query_selector(q_distinct_hop4_to_ext);
                let val_to_check =
                    meta.query_advice(next_aligned_h4_pkp_to_personid, Rotation::cur());
                let val_in_ext_table = meta.query_advice(distinct_hop4_to_ext, Rotation::cur());
                let lhs = [one.clone(), val_to_check].map(|v| v * q_pkp_active.clone());
                let rhs = [one.clone(), val_in_ext_table].map(|v| v * q_ext_table_active.clone());
                lhs.into_iter().zip(rhs).collect()
        ...
        let aligned_h4_pkp_to_personid_config = LtEqGenericChip::configure(
            meta,
            |meta| meta.query_selector(q_pkp),
            |meta| vec![meta.query_advice(aligned_h4_pkp_to_personid, Rotation::cur())],
            |meta| vec![meta.query_advice(ordered_pkp[1], Rotation::cur())],
        );
        meta.create_gate("aligned_h4_pkp_to_personid_config verify", |meta| {
            let q = meta.query_selector(q_pkp);
            vec![q * (aligned_h4_pkp_to_personid_config.is_lt(meta, None) - one.clone())]
        });

        let next_aligned_h4_pkp_to_personid_config = LtEqGenericChip::configure(
            meta,
            |meta| meta.query_selector(q_pkp),
            |meta| vec![meta.query_advice(ordered_pkp[1], Rotation::cur())],
            |meta| {
        ...
        meta.create_gate("next_aligned_h4_pkp_to_personid_config verify", |meta| {
            let q = meta.query_selector(q_pkp);
            vec![q * (next_aligned_h4_pkp_to_personid_config.is_lt(meta, None) - one.clone())]
        });

        meta.lookup_any("aligned_h4_pkp_to_personid pair lookup", |meta| {
            let q_pkp_active = meta.query_selector(q_pkp);
            let q_pairs_table_active = meta.query_selector(q_distinct_hop4_to_ext_pairs_lookup);
            let aligned_val = meta.query_advice(aligned_h4_pkp_to_personid, Rotation::cur());
            let next_aligned_val =
                meta.query_advice(next_aligned_h4_pkp_to_personid, Rotation::cur());
            let pair_first =
                meta.query_advice(distinct_hop4_to_ext_pairs_lookup_table[0], Rotation::cur());
        ...
        let h4_pkp_to_flag_config = IsZeroChip::configure(
            meta,
            |meta| meta.query_selector(q_pkp),
            |meta| {
                meta.query_advice(aligned_h4_pkp_to_personid, Rotation::cur())
                    - meta.query_advice(ordered_pkp[1], Rotation::cur())
            },
            iz_h4_pkp_to_flag,
            h4_pkp_to_flag,
        ...
        // --- Hop 5: Find friends of 4-hop friends ---
        ...
        meta.create_gate("init_h5_states", |meta| {
            let q_person_active = meta.query_selector(q_person);
            let dist5_node_assigned_val = meta.query_advice(dist5_node, Rotation::cur());
            let q_dist5_node_selector_val = meta.query_selector(q_dist5_node);
            vec![q_person_active.clone() * (dist5_node_assigned_val - q_dist5_node_selector_val)]
        });

        let q_hop5_to = meta.complex_selector();
        meta.create_gate("set_q_hop5_to_selector", |meta| {
            let q_pkp_active = meta.query_selector(q_pkp);
            let q_hop5_to_val = meta.query_selector(q_hop5_to);
            let pkp_from_is_h4_friend = meta.query_advice(h4_match_flag, Rotation::cur());
            let pkp_to_is_source = meta.query_advice(source_pkp_to_check, Rotation::cur());
            let pkp_to_is_h1_friend = meta.query_advice(h1_pkp_to_flag, Rotation::cur());
            let pkp_to_is_h2_friend = meta.query_advice(h2_pkp_to_flag, Rotation::cur());
            let pkp_to_is_h3_friend = meta.query_advice(h3_pkp_to_flag, Rotation::cur());
            let pkp_to_is_h4_friend = meta.query_advice(h4_pkp_to_flag, Rotation::cur());

            let condition = pkp_from_is_h4_friend
                * (one.clone() - pkp_to_is_source)
                * (one.clone() - pkp_to_is_h1_friend)
                * (one.clone() - pkp_to_is_h2_friend)
                * (one.clone() - pkp_to_is_h3_friend)
                * (one.clone() - pkp_to_is_h4_friend);
            vec![q_pkp_active.clone() * (q_hop5_to_val - condition)]
        });
        ...
        meta.lookup_any("distinct_hop5_to from pkp.ToId where q_hop5_to", |meta| {
            let q1 = meta.query_selector(q_distinct_hop5_to);
            let q2 = meta.query_selector(q_hop5_to);
            let a = meta.query_advice(distinct_hop5_to, Rotation::cur());
            let b = meta.query_advice(ordered_pkp[1], Rotation::cur());
            let lhs = [one.clone(), a].map(|c| c * q1.clone());
            let rhs = [one.clone(), b].map(|c| c * q2.clone());
            lhs.into_iter().zip(rhs).collect()
        ...
        let distinct_hop5_to_order = LtEqGenericChip::configure(
            meta,
            |meta| meta.query_selector(q_distinct_hop5_to_order),
            |meta| vec![meta.query_advice(distinct_hop5_to, Rotation::cur())],
            |meta| vec![meta.query_advice(distinct_hop5_to, Rotation::next())],
        );

        let iz_distinct_hop5_to_zero = meta.advice_column();
        let distinct_hop5_to_check = meta.advice_column();
        let distinct_hop5_to_zero = IsZeroChip::configure(
            meta,
            |meta| meta.query_selector(q_distinct_hop5_to_order),
            |meta: &mut VirtualCells<'_, F>| {
                meta.query_advice(distinct_hop5_to, Rotation::cur())
                    - meta.query_advice(distinct_hop5_to, Rotation::next())
            },
            iz_distinct_hop5_to_zero,
            distinct_hop5_to_check,
        );

        meta.create_gate("distinct_hop5_to_order and uniqueness", |meta| {
            let q = meta.query_selector(q_distinct_hop5_to_order);
            let check = meta.query_advice(distinct_hop5_to_check, Rotation::cur());
            let cur_lte_next = distinct_hop5_to_order.is_lt(meta, None);
            vec![q.clone() * (cur_lte_next - one.clone()), q.clone() * check]
        });

        meta.shuffle("mark 5-hop friends in person table", |meta| {
            let q_distinct_h5_active = meta.query_selector(q_distinct_hop5_to);
            let q_person_dist5_active = meta.query_selector(q_dist5_node);
            let friend_id_from_distinct_h5 = meta.query_advice(distinct_hop5_to, Rotation::cur());
            let person_id_from_person_table = meta.query_advice(person[0], Rotation::cur());
            let lhs = [one.clone(), person_id_from_person_table]
                .map(|v| v * q_person_dist5_active.clone());
            let rhs =
                [one.clone(), friend_id_from_distinct_h5].map(|v| v * q_distinct_h5_active.clone());
            lhs.into_iter().zip(rhs).collect()
        });
        ...
        meta.create_gate("calculate potential_friend_hop5", |meta| {
            ...
            let pot_h4 = meta.query_advice(potential_friend_hop4, Rotation::cur());
            let pot_h5 = meta.query_advice(potential_friend_hop5, Rotation::cur());
            vec![q_person_active.clone() * (pot_h5 - (d5.clone() + pot_h4.clone() - d5 * pot_h4))]
        });
        // --- Hop 5 Additions (Configuration) END ---
    ...
    pub fn assign(
        ...
        // --- Hop 4 Additions (Chip Construction) START ---
        ...
        // --- Hop 4 Additions (Chip Construction) END ---

        // --- Hop 5 Additions (Chip Construction) START ---
        ...
        // --- Hop 5 Additions (Chip Construction) END ---
        ...
        // --- Hop 4 Additions (Load LtEq Chips) START ---
        ...
        // --- Hop 4 Additions (Load LtEq Chips) END ---

        // --- Hop 5 Additions (Load LtEq Chips) START ---
        ...
        // --- Hop 5 Additions (Load LtEq Chips) END ---

        // BFS and data preprocessing
        let max_hops = 5; // ERWEITERT für 5 Hops
        ...
        for edge in &person_knows_person_input {
            // Use original pkp to ensure all outgoing edges from source are considered
        ...
        for edge in &person_knows_person_input {
            // Iterate over original pkp
            let pkp_source_node = edge[0];
            let pkp_target_node = edge[1];
        ...
        // --- Hop 4 Additions (Pre-computation) START ---
        ...
        // Hop 4 friends (distinct_hop4_to_table)
        let mut hop4_friends_raw = Vec::new();
        for edge in &person_knows_person_input {
            let pkp_source_node = edge[0];
            let pkp_target_node = edge[1];
            let is_pkp_source_h3 = distinct_hop3_to_table
                .binary_search(&pkp_source_node)
                .is_ok();
            if is_pkp_source_h3 {
                let is_pkp_target_source = pkp_target_node == person_id_val;
                ...
                let is_pkp_target_h2 = distinct_hop2_to_table
                    .binary_search(&pkp_target_node)
                    .is_ok();
                let is_pkp_target_h3 = distinct_hop3_to_table
                    .binary_search(&pkp_target_node)
                    .is_ok();
                if !is_pkp_target_source
                    && !is_pkp_target_h1
                    && !is_pkp_target_h2
                    && !is_pkp_target_h3
                {
        ...
        // --- Hop 4 Additions (Pre-computation) END ---

        // --- Hop 5 Additions (Pre-computation) START ---
        ...
        if distinct_hop4_to_ext_values.len() >= 2 {
            for i in 0..(distinct_hop4_to_ext_values.len() - 1) {
                distinct_hop4_to_ext_pairs_table.push((
                    distinct_hop4_to_ext_values[i],
                    distinct_hop4_to_ext_values[i + 1],
                ));
            }
        }

        // Hop 5 friends (distinct_hop5_to_table)
        let mut hop5_friends_raw = Vec::new();
        for edge in &person_knows_person_input {
            let pkp_source_node = edge[0];
            let pkp_target_node = edge[1];
            let is_pkp_source_h4 = distinct_hop4_to_table
                .binary_search(&pkp_source_node)
                .is_ok();
            if is_pkp_source_h4 {
                let is_pkp_target_source = pkp_target_node == person_id_val;
                ...
                let is_pkp_target_h3 = distinct_hop3_to_table
                    .binary_search(&pkp_target_node)
                    .is_ok();
                let is_pkp_target_h4 = distinct_hop4_to_table
                    .binary_search(&pkp_target_node)
                    .is_ok();
                if !is_pkp_target_source
                    && !is_pkp_target_h1
        ...
        // --- Hop 5 Additions (Pre-computation) END ---
        ...
        layouter.assign_region(
            ...
            |mut region| {
                ...
                for (i, p_row) in person_table.iter().enumerate() {
                    ...
                    let is_dist2 = dist == 2;
                    let is_dist3 = dist == 3;
                    let is_dist4 = dist == 4; // Added for Hop 4
                    let is_dist5 = dist == 5; // Added for Hop 5
                    ...
                    let dist4_val = F::from(is_dist4 as u64); // Added for Hop 4
                    let dist5_val = F::from(is_dist5 as u64); // Added for Hop 5
                    ...
                    region.assign_advice(
                        || format!("dist4_node at {}", i),
                        self.config.dist4_node,
                        i,
                        || Value::known(dist4_val),
                    )?; // Added for Hop 4
                    region.assign_advice(
                        || format!("dist5_node at {}", i),
                        self.config.dist5_node,
                        i,
                        || Value::known(dist5_val),
                    )?; // Added for Hop 5
                    ...
                    if is_dist5 {
                        self.config.q_dist5_node.enable(&mut region, i)?;
                    } // Added for Hop 5
                    ...
                    let pot_friend_h4_val =
                        if is_dist0 || is_dist1 || is_dist2 || is_dist3 || is_dist4 {
                            F::ONE
                        } else {
                            F::ZERO
                        }; // Added for Hop 4
                    let pot_friend_h5_val =
                        if is_dist0 || is_dist1 || is_dist2 || is_dist3 || is_dist4 || is_dist5 {
                            F::ONE
                        } else {
                            F::ZERO
                        }; // Added for Hop 5
                    ...
                    region.assign_advice(
                        || format!("potential_friend_hop4 at {}", i),
                        self.config.potential_friend_hop4,
                        i,
                        || Value::known(pot_friend_h4_val),
                    )?; // Added for Hop 4
                    region.assign_advice(
                        || format!("potential_friend_hop5 at {}", i),
                        self.config.potential_friend_hop5,
                        i,
                        || Value::known(pot_friend_h5_val),
                    )?; // Added for Hop 5
                }
                ...
                for (i, ordered_edge) in ordered_pkp_table.iter().enumerate() {
                    ...
                    // --- H1 Alignment for pkp_source_node ---
                    ...
                    // --- H1 Alignment for pkp_target_node ---
                    ...
                    // --- H2 Alignment for pkp_source_node ---
                    ...
                    // --- H2 Alignment for pkp_target_node ---
                    ...
                    // --- Hop 4 Additions (PKP Loop Assignments) START ---
                    // --- H3 Alignment for pkp_source_node ---
                    let mut aligned_val_h3_source = F::ZERO;
                    let mut next_aligned_val_h3_source = F::from(MAX_PERSON_ID);
                    let search_res_h3_src = distinct_hop3_to_ext_values
                        .binary_search_by(|p| f_to_u64(p).cmp(&f_to_u64(&pkp_source_node)));
                    match search_res_h3_src {
                        Ok(idx) => {
                            aligned_val_h3_source = distinct_hop3_to_ext_values[idx];
                            if idx + 1 < distinct_hop3_to_ext_values.len() {
                                next_aligned_val_h3_source = distinct_hop3_to_ext_values[idx + 1];
                            }
                        }
                        Err(idx) => {
                            if idx > 0 {
                                aligned_val_h3_source = distinct_hop3_to_ext_values[idx - 1];
                            }
                            if idx < distinct_hop3_to_ext_values.len() {
                                next_aligned_val_h3_source = distinct_hop3_to_ext_values[idx];
                            }
                        }
                    }
                    region.assign_advice(
                        || format!("aligned_h3_pkp_personid row {}", i),
                        self.config.aligned_h3_pkp_personid,
                        i,
                        || Value::known(aligned_val_h3_source),
                    )?;
                    region.assign_advice(
                        || format!("next_aligned_h3_pkp_personid row {}", i),
                        self.config.next_aligned_h3_pkp_personid,
                        i,
                        || Value::known(next_aligned_val_h3_source),
                    )?;
                    aligned_h3_pkp_personid_chip
                        .assign(&mut region, i, &[aligned_val_h3_source], &[pkp_source_node])
                        .unwrap();
                    next_aligned_h3_pkp_personid_chip
                        .assign(
                            ...
                            i,
                            &[pkp_source_node],
                            &[next_aligned_val_h3_source - F::ONE],
                        )
                        .unwrap();
                    let diff_h3_match_from = aligned_val_h3_source - pkp_source_node;
                    h3_match_flag_chip
                        .assign(&mut region, i, Value::known(diff_h3_match_from))
                        .unwrap();
                    let h3_match_flag_val = if diff_h3_match_from == F::ZERO {
                        F::ONE
                    } else {
                        F::ZERO
                    };
                    region.assign_advice(
                        || format!("h3_match_flag row {}", i),
                        self.config.h3_match_flag,
                        i,
                        || Value::known(h3_match_flag_val),
                    )?;

                    // --- H3 Alignment for pkp_target_node ---
                    let mut aligned_val_h3_target = F::ZERO;
                    let mut next_aligned_val_h3_target = F::from(MAX_PERSON_ID);
                    let search_res_h3_target = distinct_hop3_to_ext_values
                        .binary_search_by(|p| f_to_u64(p).cmp(&f_to_u64(&pkp_target_node)));
                    match search_res_h3_target {
                        Ok(idx) => {
                            aligned_val_h3_target = distinct_hop3_to_ext_values[idx];
                            if idx + 1 < distinct_hop3_to_ext_values.len() {
                                next_aligned_val_h3_target = distinct_hop3_to_ext_values[idx + 1];
                            }
                        }
                        Err(idx) => {
                            if idx > 0 {
                                aligned_val_h3_target = distinct_hop3_to_ext_values[idx - 1];
                            }
                            if idx < distinct_hop3_to_ext_values.len() {
                                next_aligned_val_h3_target = distinct_hop3_to_ext_values[idx];
                            }
                        }
                    }
                    region.assign_advice(
                        || format!("aligned_h3_pkp_to_personid row {}", i),
                        self.config.aligned_h3_pkp_to_personid,
                        i,
                        || Value::known(aligned_val_h3_target),
                    )?;
                    region.assign_advice(
                        || format!("next_aligned_h3_pkp_to_personid row {}", i),
                        self.config.next_aligned_h3_pkp_to_personid,
                        i,
                        || Value::known(next_aligned_val_h3_target),
                    )?;
                    aligned_h3_pkp_to_personid_chip
                        .assign(&mut region, i, &[aligned_val_h3_target], &[pkp_target_node])
                        .unwrap();
                    next_aligned_h3_pkp_to_personid_chip
                        .assign(
                            ...
                            i,
                            &[pkp_target_node],
                            &[next_aligned_val_h3_target - F::ONE],
                        )
                        .unwrap();
                    let diff_h3_pkp_to = aligned_val_h3_target - pkp_target_node;
                    h3_pkp_to_flag_chip
                        .assign(&mut region, i, Value::known(diff_h3_pkp_to))
                        .unwrap();
                    let h3_pkp_to_flag_val = if diff_h3_pkp_to == F::ZERO {
                        F::ONE
                    } else {
                        F::ZERO
                    };
                    region.assign_advice(
                        || format!("h3_pkp_to_flag row {}", i),
                        self.config.h3_pkp_to_flag,
                        i,
                        || Value::known(h3_pkp_to_flag_val),
                    )?;

                    region.assign_advice(
                        || {
                            format!(
                                "placeholder_distinct_hop3_ext_for_pkp_source_align row {}",
                                i
                            )
                        },
                    ...
                    region.assign_advice(
                        || {
                            format!(
                                "placeholder_distinct_hop3_ext_for_pkp_target_align row {}",
                                i
                            )
                        },
                        self.config.distinct_hop3_to_ext,
                    ...
                    // Enable q_hop4_to selector
                    let enable_q_hop4_val_f = h3_match_flag_val
                        * (F::ONE - source_pkp_to_check_val)
                        * (F::ONE - h1_pkp_to_flag_val)
                        * (F::ONE - h2_pkp_to_flag_val)
                        * (F::ONE - h3_pkp_to_flag_val);
                    if enable_q_hop4_val_f == F::ONE {
                        self.config.q_hop4_to.enable(&mut region, i)?;
                    }
                    // --- Hop 4 Additions (PKP Loop Assignments) END ---

                    // --- Hop 5 Additions (PKP Loop Assignments) START ---
                    // --- H4 Alignment for pkp_source_node ---
                    let mut aligned_val_h4_source = F::ZERO;
                    let mut next_aligned_val_h4_source = F::from(MAX_PERSON_ID);
                    let search_res_h4_src = distinct_hop4_to_ext_values
                        .binary_search_by(|p| f_to_u64(p).cmp(&f_to_u64(&pkp_source_node)));
                    match search_res_h4_src {
                        Ok(idx) => {
                            aligned_val_h4_source = distinct_hop4_to_ext_values[idx];
                            if idx + 1 < distinct_hop4_to_ext_values.len() {
                                next_aligned_val_h4_source = distinct_hop4_to_ext_values[idx + 1];
                            }
                        }
                        Err(idx) => {
                            if idx > 0 {
                                aligned_val_h4_source = distinct_hop4_to_ext_values[idx - 1];
                            }
                            if idx < distinct_hop4_to_ext_values.len() {
                                next_aligned_val_h4_source = distinct_hop4_to_ext_values[idx];
                            }
                        }
                    }
                    region.assign_advice(
                        || format!("aligned_h4_pkp_personid row {}", i),
                        self.config.aligned_h4_pkp_personid,
                        i,
                        || Value::known(aligned_val_h4_source),
                    )?;
                    region.assign_advice(
                        || format!("next_aligned_h4_pkp_personid row {}", i),
                        self.config.next_aligned_h4_pkp_personid,
                        i,
                        || Value::known(next_aligned_val_h4_source),
                    )?;
                    aligned_h4_pkp_personid_chip
                        .assign(&mut region, i, &[aligned_val_h4_source], &[pkp_source_node])
                        .unwrap();
                    next_aligned_h4_pkp_personid_chip
                        .assign(
                            &mut region,
                            i,
                            &[pkp_source_node],
                            &[next_aligned_val_h4_source - F::ONE],
                        )
                        .unwrap();
                    let diff_h4_match_from = aligned_val_h4_source - pkp_source_node;
                    h4_match_flag_chip
                        .assign(&mut region, i, Value::known(diff_h4_match_from))
                        .unwrap();
                    let h4_match_flag_val = if diff_h4_match_from == F::ZERO {
                        F::ONE
                    } else {
                        F::ZERO
                    };
                    region.assign_advice(
                        || format!("h4_match_flag row {}", i),
                        self.config.h4_match_flag,
                        i,
                        || Value::known(h4_match_flag_val),
                    )?;

                    // --- H4 Alignment for pkp_target_node ---
                    let mut aligned_val_h4_target = F::ZERO;
                    let mut next_aligned_val_h4_target = F::from(MAX_PERSON_ID);
                    let search_res_h4_target = distinct_hop4_to_ext_values
                        .binary_search_by(|p| f_to_u64(p).cmp(&f_to_u64(&pkp_target_node)));
                    match search_res_h4_target {
                        Ok(idx) => {
                            aligned_val_h4_target = distinct_hop4_to_ext_values[idx];
                            if idx + 1 < distinct_hop4_to_ext_values.len() {
                                next_aligned_val_h4_target = distinct_hop4_to_ext_values[idx + 1];
                            }
                        }
                        Err(idx) => {
                            if idx > 0 {
                                aligned_val_h4_target = distinct_hop4_to_ext_values[idx - 1];
                            }
                            if idx < distinct_hop4_to_ext_values.len() {
                                next_aligned_val_h4_target = distinct_hop4_to_ext_values[idx];
                            }
                        }
                    }
                    region.assign_advice(
                        || format!("aligned_h4_pkp_to_personid row {}", i),
                        self.config.aligned_h4_pkp_to_personid,
                        i,
                        || Value::known(aligned_val_h4_target),
                    )?;
                    region.assign_advice(
                        || format!("next_aligned_h4_pkp_to_personid row {}", i),
                        self.config.next_aligned_h4_pkp_to_personid,
                        i,
                        || Value::known(next_aligned_val_h4_target),
                    )?;
                    aligned_h4_pkp_to_personid_chip
                        .assign(&mut region, i, &[aligned_val_h4_target], &[pkp_target_node])
                        .unwrap();
                    next_aligned_h4_pkp_to_personid_chip
                        .assign(
                            &mut region,
                            i,
                            &[pkp_target_node],
                            &[next_aligned_val_h4_target - F::ONE],
                        )
                        .unwrap();
                    let diff_h4_pkp_to = aligned_val_h4_target - pkp_target_node;
                    h4_pkp_to_flag_chip
                        .assign(&mut region, i, Value::known(diff_h4_pkp_to))
                        .unwrap();
                    let h4_pkp_to_flag_val = if diff_h4_pkp_to == F::ZERO {
                        F::ONE
                    } else {
                        F::ZERO
                    };
                    region.assign_advice(
                        || format!("h4_pkp_to_flag row {}", i),
                        self.config.h4_pkp_to_flag,
                        i,
                        || Value::known(h4_pkp_to_flag_val),
                    )?;

                    region.assign_advice(
                        || {
                            format!(
                                "placeholder_distinct_hop4_ext_for_pkp_source_align row {}",
                                i
                            )
                        },
                        self.config.distinct_hop4_to_ext,
                        i,
                        || Value::known(aligned_val_h4_source),
                    )?;
                    region.assign_advice(
                        || {
                            format!(
                                "placeholder_distinct_hop4_ext_for_pkp_target_align row {}",
                                i
                            )
                        },
                        self.config.distinct_hop4_to_ext,
                    ...
                    if enable_q_hop5_val_f == F::ONE {
                        self.config.q_hop5_to.enable(&mut region, i)?;
                    }
                    // --- Hop 5 Additions (PKP Loop Assignments) END ---
                }
                ...
                // Assign distinct_hop1_to_ext table
                ...
                // Assign distinct_hop2_to_ext table
                ...
                // --- Hop 4 Additions (Table Assignments) START ---
                // Assign distinct_hop3_to_ext table
                for (i, &ext_id) in distinct_hop3_to_ext_values.iter().enumerate() {
                    self.config.q_distinct_hop3_to_ext.enable(&mut region, i)?;
                    region.assign_advice(
                        || format!("authoritative_distinct_hop3_to_ext row {}", i),
                        self.config.distinct_hop3_to_ext,
                        i,
                        || Value::known(ext_id),
                    )?;
                ...
                // Assign distinct_hop3_to_ext_pairs_lookup_table
                for (i, &(pair_first, pair_second)) in
                    distinct_hop3_to_ext_pairs_table.iter().enumerate()
                {
                    self.config
                        .q_distinct_hop3_to_ext_pairs_lookup
                        .enable(&mut region, i)?;
                    region.assign_advice(
                        || format!("d_h3_ext_pairs_lookup_tab[0] row {}", i),
                        self.config.distinct_hop3_to_ext_pairs_lookup_table[0],
                        i,
                        || Value::known(pair_first),
                    )?;
                    region.assign_advice(
                        || format!("d_h3_ext_pairs_lookup_tab[1] row {}", i),
                ...
                // Assign distinct_hop4_to table
                for (i, &h4_friend_id) in distinct_hop4_to_table.iter().enumerate() {
                    self.config.q_distinct_hop4_to.enable(&mut region, i)?;
                    region.assign_advice(
                        || format!("distinct_hop4_to row {}", i),
                        self.config.distinct_hop4_to,
                        i,
                        || Value::known(h4_friend_id),
                    )?;
                    if i < distinct_hop4_to_table.len() - 1 {
                        self.config
                            .q_distinct_hop4_to_order
                            .enable(&mut region, i)?;
                        let next_h4_friend_id = distinct_hop4_to_table[i + 1];
                        distinct_hop4_to_order_chip
                            .assign(&mut region, i, &[h4_friend_id], &[next_h4_friend_id])
                            .unwrap();
                        let diff_distinct_h4 = h4_friend_id - next_h4_friend_id;
                        distinct_hop4_to_zero_chip
                            .assign(&mut region, i, Value::known(diff_distinct_h4))
                            .unwrap();
                        let distinct_hop4_to_check_val = if diff_distinct_h4 == F::ZERO {
                            F::ONE
                        } else {
                            F::ZERO
                        };
                        region.assign_advice(
                            || format!("distinct_hop4_to_check row {}", i),
                            self.config.distinct_hop4_to_check,
                            i,
                            || Value::known(distinct_hop4_to_check_val),
                        )?;
                    }
                }
                // --- Hop 4 Additions (Table Assignments) END ---

                // --- Hop 5 Additions (Table Assignments) START ---
                // Assign distinct_hop4_to_ext table
                for (i, &ext_id) in distinct_hop4_to_ext_values.iter().enumerate() {
                    self.config.q_distinct_hop4_to_ext.enable(&mut region, i)?;
                    region.assign_advice(
                        || format!("authoritative_distinct_hop4_to_ext row {}", i),
                        self.config.distinct_hop4_to_ext,
                        i,
                        || Value::known(ext_id),
                    )?;
                    ...
                    if i < distinct_hop4_to_ext_values.len() - 1 {
                        self.config
                            .q_distinct_hop4_to_ext_order
                            .enable(&mut region, i)?;
                        distinct_hop4_to_ext_order_chip
                            .assign(
                                &mut region,
                                i,
                                &[ext_id],
                ...
                // Assign distinct_hop4_to_ext_pairs_lookup_table
                for (i, &(pair_first, pair_second)) in
                    distinct_hop4_to_ext_pairs_table.iter().enumerate()
                {
                    self.config
                        .q_distinct_hop4_to_ext_pairs_lookup
                        .enable(&mut region, i)?;
                    region.assign_advice(
                        || format!("d_h4_ext_pairs_lookup_tab[0] row {}", i),
                        self.config.distinct_hop4_to_ext_pairs_lookup_table[0],
                        i,
                        || Value::known(pair_first),
                    )?;
                    region.assign_advice(
                        || format!("d_h4_ext_pairs_lookup_tab[1] row {}", i),
                        self.config.distinct_hop4_to_ext_pairs_lookup_table[1],
                        i,
                        || Value::known(pair_second),
                    )?;
                }

                // Assign distinct_hop5_to table
                for (i, &h5_friend_id) in distinct_hop5_to_table.iter().enumerate() {
                    self.config.q_distinct_hop5_to.enable(&mut region, i)?;
                    region.assign_advice(
                        || format!("distinct_hop5_to row {}", i),
                        self.config.distinct_hop5_to,
                        i,
                        || Value::known(h5_friend_id),
                    )?;
                    if i < distinct_hop5_to_table.len() - 1 {
                        self.config
                            .q_distinct_hop5_to_order
                            .enable(&mut region, i)?;
                        let next_h5_friend_id = distinct_hop5_to_table[i + 1];
                        distinct_hop5_to_order_chip
                            .assign(&mut region, i, &[h5_friend_id], &[next_h5_friend_id])
                            .unwrap();
                        let diff_distinct_h5 = h5_friend_id - next_h5_friend_id;
                        distinct_hop5_to_zero_chip
                            .assign(&mut region, i, Value::known(diff_distinct_h5))
                            .unwrap();
                        let distinct_hop5_to_check_val = if diff_distinct_h5 == F::ZERO {
                            F::ONE
                        } else {
                            F::ZERO
                        };
                        region.assign_advice(
                            || format!("distinct_hop5_to_check row {}", i),
                            self.config.distinct_hop5_to_check,
                            i,
                            || Value::known(distinct_hop5_to_check_val),
                        )?;
                    }
                }
                // --- Hop 5 Additions (Table Assignments) END ---
...
mod tests {
    ...
    mod utils {
        ...
        pub fn string_to_u64(s: &str) -> u64 {
            // A simple hash-like function for testing, replace with your actual logic if needed
            s.chars().fold(0, |acc, c| acc.wrapping_add(c as u64))
        }
        pub fn parse_date(s: &str) -> u64 {
            // Simplified stub, e.g., "2023-01-01" -> 20230101
            s.replace("-", "")
                .parse()
                .unwrap_or_else(|_| s.len() as u64)
        }
        pub fn parse_datetime(s: &str) -> u64 {
            // Simplified stub, e.g., "2023-01-01T12:00:00" -> 20230101120000
            s.replace("-", "")
                .replace("T", "")
                .replace(":", "")
                .parse()
        ...
        pub fn ipv4_to_u64(s: &str) -> u64 {
            // Simplified stub for "192.168.1.1"
            s.split('.').enumerate().fold(0, |acc, (i, part)| {
                acc + (part.parse::<u64>().unwrap_or(0) << ((3 - i) * 8))
            })
        }
    }
    ...
    fn test_is1_circuit_4hop() {
        ...
        for row in person_data.iter() {
            person_table.push(vec![
                Fr::from(row[0].parse::<u64>().unwrap()),
        ...
        for row in relation_data.iter() {
            person_knows_person.push(vec![
                Fr::from(row[0].parse::<u64>().unwrap()),
                Fr::from(row[1].parse::<u64>().unwrap()),
            ]);
        }
        println!(
            "person_table.len: {:?}, pkp.len: {:?}",
            person_table.len(),
            person_knows_person.len()
        );
