...
pub struct Is1CircuitConfig<F: Field + Ord + std::hash::Hash> {
    ...
    potential_friend_hop3: Column<Advice>, // 新增: 第三跳潜在朋友
    dist0_node: Column<Advice>,
    dist1_node: Column<Advice>,
    q_dist1_node: Selector,
    dist2_node: Column<Advice>,
    q_dist2_node: Selector,
    dist3_node: Column<Advice>, // 新增: 距离为3的节点
    q_dist3_node: Selector,     // 新增: 距离为3的节点选择器
...
impl<F: Field + Ord + std::hash::Hash> Is1Chip<F> {
    ...
    pub fn configure(meta: &mut ConstraintSystem<F>) -> Is1CircuitConfig<F> {
        ...
        meta.create_gate("q_dist3_node", |meta| {
            let q = meta.query_selector(q_person);
            let potential_friend = meta.query_advice(dist3_node, Rotation::cur());
            let q1 = meta.query_selector(q_dist3_node);
            vec![q.clone() * (potential_friend - q1)]
        });
        ...
        let aligned_h2_pkp_personid_config = LtEqGenericChip::configure(
            meta,
            |meta| meta.query_selector(q_pkp),
            |meta| vec![meta.query_advice(aligned_h2_pkp_personid, Rotation::cur())], // aligned hop2 friend ID
            |meta| vec![meta.query_advice(ordered_pkp[0], Rotation::cur())], // actual Person.id
        );
        ...
        meta.create_gate("set_q_hop3_to_selector", |meta| {
            let q = meta.query_selector(q_pkp);
            let q_hop3 = meta.query_selector(q_hop3_to);
            let h2_match_from = meta.query_advice(h2_match_flag, Rotation::cur());
            let h1_match_to = meta.query_advice(h1_pkp_to_flag, Rotation::cur());
            let source_match_to = meta.query_advice(source_pkp_to_check, Rotation::cur());
            vec![
                q * (q_hop3
        ...
        // 第三跳的唯一节点表
        meta.lookup_any("distinct_hop3_to1", |meta| {
        ...
        meta.lookup_any("distinct_hop3_to2", |meta| {
            let q1 = meta.query_selector(q_distinct_hop3_to);
            let q2 = meta.query_selector(q_hop3_to);
            let a = meta.query_advice(distinct_hop3_to, Rotation::cur());
            let b = meta.query_advice(ordered_pkp[1], Rotation::cur());
        ...
        meta.create_gate("distinct_hop3_to_order", |meta| {
            let q = meta.query_selector(q_distinct_hop3_to_order);
            let check = meta.query_advice(distinct_hop3_to_check, Rotation::cur());
            vec![q.clone() * check]
        });

        // 将唯一的第三跳节点与 dist3_node 进行匹配
        meta.shuffle("dist3_node", |meta| {
            let q1 = meta.query_selector(q_distinct_hop3_to);
            let q2 = meta.query_selector(q_dist3_node);
            let a = meta.query_advice(distinct_hop3_to, Rotation::cur());
            let b = meta.query_advice(person[0], Rotation::cur());
            let lhs = [one.clone(), a].map(|c| c * q1.clone());
            let rhs = [one.clone(), b].map(|c| c * q2.clone());
        ...
        // 更新潜在朋友的逻辑，加入第三跳
        meta.create_gate("hop3_status", |meta| {
            let q = meta.query_selector(q_person);

            let d3 = meta.query_advice(dist3_node, Rotation::cur());
            let p2 = meta.query_advice(potential_friend_hop2, Rotation::cur());
            let p3 = meta.query_advice(potential_friend_hop3, Rotation::cur());
            vec![q.clone() * (p3 - (d3.clone() + p2.clone() - d3 * p2))]
        });
    ...
    pub fn assign(
        ...
        // BFS and data preprocessing (更新最大跳数)
        let max_hops = 3; // 更新为3跳
        ...
        if distances.contains_key(&person_id_val) {
            ...
            println!(
                "警告: BFS 源 Person ID {:?} 未在 person_table 中找到。",
                person_id_val
            );
        }
        ...
        let distinct_hop2_to_table = hop2_friends_from_pkp_raw.clone();

        // 新增: 构建第三跳朋友列表
        let mut hop3_friends_from_pkp_raw = Vec::new();
        for ordered_edge in &ordered_pkp_table {
            let pkp_source_node = ordered_edge[0];
            let pkp_target_node = ordered_edge[1];

            let is_pkp_source_a_2hop_friend = distinct_hop2_to_table
                .binary_search(&pkp_source_node)
                .is_ok();
            if is_pkp_source_a_2hop_friend {
                let is_pkp_target_person_id = pkp_target_node == person_id_val;
                let is_pkp_target_a_1hop_friend = hop1_friends_distinct_sorted
                    .binary_search(&pkp_target_node)
                    .is_ok();
                let is_pkp_target_a_2hop_friend = distinct_hop2_to_table
                    .binary_search(&pkp_target_node)
                    .is_ok();

                if !is_pkp_target_person_id
                    && !is_pkp_target_a_1hop_friend
                    && !is_pkp_target_a_2hop_friend
                {
                    let bfs_dist_to_target = distances
                        .get(&pkp_target_node)
                        .cloned()
                    ...
                    if bfs_dist_to_target == 3 {
                        hop3_friends_from_pkp_raw.push(pkp_target_node);
                    }
                }
            }
        }
        hop3_friends_from_pkp_raw.sort_by(|a, b| f_to_u64(a).cmp(&f_to_u64(b)));
        hop3_friends_from_pkp_raw.dedup();
        let distinct_hop3_to_table = hop3_friends_from_pkp_raw;
        ...
        layouter.assign_region(
            ...
            |mut region| {
                ...
                for (i, ordered_edge) in ordered_pkp_table.iter().enumerate() {
                    ...
                    // 新增: 第二跳对齐和匹配检查
                    // 检查pkp_source_node是否为第二跳节点
                    let is_pkp_source_a_2hop_friend = distinct_hop2_to_table
                        .binary_search_by(|p| f_to_u64(p).cmp(&f_to_u64(&pkp_source_node)))
                        .is_ok();
                    let mut aligned_h2_val_for_source = F::ZERO;
                    let mut next_aligned_h2_val_for_source = F::from(MAX_PERSON_ID);

                    if is_pkp_source_a_2hop_friend {
                        aligned_h2_val_for_source = pkp_source_node;
                    } else if !distinct_hop2_to_table.is_empty() {
                        let insert_idx = distinct_hop2_to_table
                            .binary_search_by(|p| f_to_u64(p).cmp(&f_to_u64(&pkp_source_node)))
                            .err()
                            .unwrap();
                        if insert_idx > 0 {
                            aligned_h2_val_for_source = distinct_hop2_to_table[insert_idx - 1];
                        }
                        if insert_idx < distinct_hop2_to_table.len() {
                            next_aligned_h2_val_for_source = distinct_hop2_to_table[insert_idx];
                        }
                    }
                    ...
                    next_aligned_h2_pkp_personid_chip
                        .assign(
                            ...
                            i,
                            &[pkp_source_node],
                            &[next_aligned_h2_val_for_source - F::ONE],
                        )
                        .unwrap();

                    // h2_match_flag: pkp_source_node is a 2-hop friend
                    let diff_h2_match_from = aligned_h2_val_for_source - pkp_source_node;
                    h2_match_flag_chip
                    ...
                    // 新增: 启用第三跳选择器
                    let enable_q_hop3_val = h2_match_flag_val
                        * (F::ONE - h1_pkp_to_flag_val)
                        * (F::ONE - source_pkp_to_check_val);
                    if enable_q_hop3_val == F::ONE {
