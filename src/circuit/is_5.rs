...
impl<F: Field + Ord> Is5chip<F> {
    ...
    pub fn assign(
        ...
        comment_hasCreator_person: Vec<Vec<u64>>, // 从F改为u64
        post_hasCreator_person: Vec<Vec<u64>>,    // 从F改为u64
        person: Vec<Vec<u64>>,                    // 从F改为u64
        message_id: u64,                          // 从F改为u64
    ) -> Result<(), Error> {
            ...
            |mut region| {
                for (i, row) in comment_hasCreator_person.iter().enumerate() {
                    ...
                    for j in 0..comment_hasCreator_person[0].len() {
                        region.assign_advice(
                            ...
                            i,
                            || Value::known(F::from(row[j])), // 使用F::from转换u64为F
                        )?;
                    }
                    region.assign_advice(
                        || "comment_check",
                        ...
                        i,
                        || Value::known(F::from(comment_check_bits[i])), // 使用F::from转换u64为F
                    )?;
                    region.assign_advice(
                        || "messageid",
                        self.config.message_id,
                        i,
                        || Value::known(F::from(message_id)), // 使用F::from转换u64为F
                    )?;
                    self.config.q_commentid.enable(&mut region, i)?;
                    let diff = F::from(row[0]) - F::from(message_id); // 计算差值，使用F::from转换
                    comment_eq
                ...
                for (i, row) in post_hasCreator_person.iter().enumerate() {
                    ...
                    region.assign_advice(
                        ...
                        i,
                        || Value::known(F::from(post_check_bits[i])), // 使用F::from转换u64为F
                    )?;
                    region.assign_advice(
                        || "messageid",
                        self.config.message_id,
                ...
                for i in 0..3 {
                    region.assign_advice(
                        ...
                        0,
                        || Value::known(F::from(res_comment[i])), // 使用F::from转换u64为F
                    )?;
                    region.assign_advice(
                        || format!("picked_post row {} col {}", 0, i),
                        self.config.picked_post[i],
                        0,
                        || Value::known(F::from(res_post[i])), // 使用F::from转换u64为F
                    )?;
                }
                ...
                for i in 0..3 {
                    region.assign_advice(
                        ...
                        0,
                        || Value::known(F::from(res_person[i])), // 使用F::from转换u64为F
                    )?;
                }
...
pub struct MyCircuit<F: Field + Ord> {
    pub comment_hasCreator_person: Vec<Vec<u64>>, // 修改为u64
    pub post_hasCreator_person: Vec<Vec<u64>>,    // 修改为u64
    pub message_id: u64,                          // 修改为u64
    pub person: Vec<Vec<u64>>,                    // 修改为u64
...
impl<F: Copy + Default + Field + Ord> Default for MyCircuit<F> {
    fn default() -> Self {
        Self {
            ...
            message_id: Default::default(), // 这里会使用u64默认值0
            person: Vec::new(),
            _marker: PhantomData,
        }
    }
