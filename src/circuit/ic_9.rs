...
pub struct ic9CircuitConfig<F: Field + Ord + std::hash::Hash> {
    ...
    // 用于边 (U,V) from person_knows_person 的规范化
    ...
    // 用于BFS路径中的边 (P,C) (predecessor, current_node) 的规范化
...
impl<F: Field + Ord + std::hash::Hash> ic9Chip<F> {
    ...
    pub fn configure(meta: &mut ConstraintSystem<F>) -> ic9CircuitConfig<F> {
        ...
        // 规范化 pkp_norm_0, pkp_norm_1 来自 pkp[0], pkp[1]
        // q_edge 将作为此规范化逻辑的激活选择器
        let pkp_norm_order_config = LtEqGenericChip::configure(
            meta,
            |meta| meta.query_selector(q_edge), // 激活时机：当处理 pkp 表的行时
        ...
        meta.create_gate("pkp_normalization", |meta| {
            ...
            let order_check = pkp_norm_order_config.is_lt(meta, None); // 要求 norm_v < norm_u (M < m) 为 false (0)

            vec![
                q.clone() * sum_check,
                q.clone() * prod_check,
                q * order_check, // M < m 必须为 false
            ]
        });

        // 规范化 pc_norm_0, pc_norm_1 来自 predecessor, person[0]
        // q_enable_pc_normalization 将作为此规范化逻辑的激活选择器
        let pc_norm_order_config = LtEqGenericChip::configure(
            meta,
            |meta| meta.query_selector(q_enable_pc_normalization), // 激活时机：当处理 person 表中需要验证边的行时
        ...
        meta.create_gate("pc_normalization", |meta| {
            ...
            let order_check = pc_norm_order_config.is_lt(meta, None); // 要求 norm_c < norm_p (M < m) 为 false (0)
        ...
        let last_vs_top_date_zero = IsZeroChip::configure(
            ...
            |meta| {
                // 检查当前消息日期是否等于 last_top20_message 的日期
                meta.query_advice(top20[5], Rotation::cur())
        ...
        let last_vs_remain_date_zero = IsZeroChip::configure(
            ...
            |meta| {
                // 检查当前候选消息日期是否等于 last_top20_message 的日期
        ...
        let message_date_config = LtEqGenericChip::configure(
            ...
            |meta| vec![meta.query_advice(candidate[1], Rotation::cur())], // 消息日期
            |meta| vec![meta.query_advice(max_date, Rotation::cur())],     // 最大日期
        );
        ...
        meta.create_gate("date filter constraint", |meta| {
            ...
            // 如果日期 <= max_date，则消息必须被处理 (processed = 1)
            // 如果日期 > max_date，则消息不能被处理 (processed = 0)
        ...
        meta.create_gate("message processing completeness", |meta| {
            ...
            // 如果消息被处理，它必须出现在top20或remain中的一个
    ...
    pub fn assign(
        ...
        let mut distances: HashMap<u64, u64> = HashMap::new(); // 存储节点到源点的距离
        let mut predecessors: HashMap<u64, u64> = HashMap::new(); // 存储每个节点的前驱节点
        let mut q: VecDeque<u64> = VecDeque::new(); // BFS队列

        // 初始化所有person_table中节点的距离为虚拟距离
        ...
        if distances.contains_key(&person_id_val) {
            ...
            println!("警告: BFS源Person ID未在person_table中找到。");
        }
        ...
        // 创建扩展的朋友ID表
        let mut friendsid_ext_values = vec![0u64]; // 添加0作为边界
        for &id in &friends_within_two_hops {
            friendsid_ext_values.push(id);
        }
        friendsid_ext_values.push(MAX_PERSON_ID); // 添加最大值作为边界
        ...
        if !candidate_messages.is_empty() {
            for (i, msg) in candidate_messages.iter().enumerate() {
                if i < 20 {
                    ...
                    if i == 19 {
                        last_top20_message = vec![msg[0], msg[1]]; // 存储最后一个top20消息的ID和日期
                    }
                }
            }
            ...
            |mut region| {
                ...
                for (i, row) in person_table.iter().enumerate() {
                    ...
                    let (predecessor_val, predecessor_dist) = if row[0] == person_id_val {
                        ...
                        // 可达节点(非源点)：前驱是BFS找到的predecessor，前驱距离是dist-1
                ...
                for (i, edge) in person_knows_person.iter().enumerate() {
                    ...
                    // 分配给target_less_chip检查target_dist <= source_dist + 1
                    target_less_chip
                        .assign(
                            &mut region,
                            i,
                ...
                for (i, &(message_id, creator_id)) in ordered_comment_creators.iter().enumerate() {
                    ...
                    if i < ordered_comment_creators.len() - 1 {
                        ...
                        // 分配给ordered_comment_creator_person_id_sort_chip
                        ordered_comment_creator_person_id_sort_chip
                            .assign(
                                &mut region,
                                i,
                    ...
                    // 分配给align_comment_friend_id_le_person_id_chip
                    align_comment_friend_id_le_person_id_chip
                        .assign(
                            &mut region,
                            i,
                    ...
                    // 分配给person_id_lt_next_aligned_comment_friend_id_chip
                    person_id_lt_next_aligned_comment_friend_id_chip
                        .assign(
                            &mut region,
                            i,
                ...
                for (i, &(message_id, creator_id)) in ordered_post_creators.iter().enumerate() {
                    ...
                    if i < ordered_post_creators.len() - 1 {
                        ...
                        // 分配给ordered_post_creator_person_id_sort_chip
                        ordered_post_creator_person_id_sort_chip
                            .assign(
                                &mut region,
                                i,
                    ...
                    // 分配给align_post_friend_id_le_person_id_chip
                    align_post_friend_id_le_person_id_chip
                        .assign(
                            &mut region,
                            i,
                    ...
                    // 分配给person_id_lt_next_aligned_post_friend_id_chip
                    person_id_lt_next_aligned_post_friend_id_chip
                        .assign(
                            &mut region,
                            i,
                ...
                for (i, row) in top20_messages.iter().enumerate() {
                    ...
                    // 找到相应的创建者信息
                    let message_id = row[0];
                    let message_type = row[2]; // 0=评论，1=帖子
                    ...
                    if message_type == 0 {
                        ...
                        for entry in &comment_hasCreator_person {
                            if entry[0] == message_id {
                                creator_info = vec![entry[1]]; // 只需要创建者ID
                                break;
                            }
                        }
                    } else {
                    ...
                    // messageContent 和 messageImageFile
                    let content_val;
                    let image_val;
                    ...
                    region.assign_advice(
                        || format!("top20[4] (imageFile) at {}", i), // 应该是 imageFile
                        self.config.top20[4],
                        i,
                        || Value::known(F::from(image_val)), // 赋 image_val
                    )?;
                    ...
                    region.assign_advice(
                        ...
                        self.config.top20[5],
                        i,
                        || Value::known(F::from(row[1])), // row[1] 是 candidate_messages 的 creationDate
                    )?;

                    region.assign_advice(
                        || format!("top20[6] (content) at {}", i), // 应该是 content
                        self.config.top20[6],
                        i,
                        || Value::known(F::from(content_val)), // 赋 content_val
                    )?;
                    ...
                    // 检查当前消息日期是否等于last_top20_message日期
                ...
                // 为remain消息检查与last_top20_message的关系
                for (i, row) in candidate_messages.iter().enumerate() {
