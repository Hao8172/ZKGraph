...
// use crate::data::csr::CsrValue; // Nicht verwendet
use halo2_proofs::halo2curves::ff::PrimeField;
use halo2_proofs::{circuit::*, plonk::*, poly::Rotation};
use std::collections::{HashMap, VecDeque}; // BTreeSet, HashSet nicht explizit im Code verwendet, aber gut für Vorverarbeitung
...
pub struct Is1CircuitConfig<F: Field + Ord + std::hash::Hash> {
    ...
    h1_match_flag: Column<Advice>, // Is PKP source a 1-hop friend?
    ...
    h1_pkp_to_flag: Column<Advice>, // Is PKP target a 1-hop friend?
    ...
    h2_match_flag: Column<Advice>, // Is PKP source a 2-hop friend?
    ...
    h2_pkp_to_flag: Column<Advice>, // Is PKP target a 2-hop friend?
    ...
    // --- Hop 3 Alignment with PKP (for Hop 4 calculations) ---
    ...
    h3_match_flag: Column<Advice>, // Is PKP source a 3-hop friend?
    ...
    h3_pkp_to_flag: Column<Advice>, // Is PKP target a 3-hop friend?
...
impl<F: Field + Ord + std::hash::Hash> Is1Chip<F> {
    ...
    pub fn configure(meta: &mut ConstraintSystem<F>) -> Is1CircuitConfig<F> {
        ...
        // --- Hop 2 Configuration (similar to Hop 1) ---
        ...
        // --- distinct_hop2_to_ext Configuration ---
        ...
        // --- H2 Alignment with PKP (for Hop 3 calculations) ---
        ...
        // --- distinct_hop3_to_ext Configuration ---
        ...
        // --- H3 Alignment with PKP (for Hop 4 calculations) ---
    ...
    pub fn assign(
        ...
        distinct_hop3_to_ext_order_chip.load(layouter).unwrap(); // H4 load
        aligned_h3_pkp_personid_chip.load(layouter).unwrap();
        next_aligned_h3_pkp_personid_chip.load(layouter).unwrap(); // H4 load
        aligned_h3_pkp_to_personid_chip.load(layouter).unwrap();
        next_aligned_h3_pkp_to_personid_chip.load(layouter).unwrap(); // H4 load
        distinct_hop4_to_order_chip.load(layouter).unwrap(); // H4 load

        // --- BFS and Data Preprocessing ---
        let max_hops = 4; // MODIFIED for 4 hops
        ...
        // Hop 1 friends (distinct_hop1_to_table) & ext
        let mut hop1_friends_vec = Vec::new();
        for edge in &person_knows_person_input {
            // Using original input as BFS is the source of truth for distances
        ...
        // Hop 2 friends (distinct_hop2_to_table) & ext
        let mut hop2_friends_raw = Vec::new();
        for edge in &person_knows_person_input {
            let pkp_s = edge[0];
            let pkp_t = edge[1];
            if distinct_hop1_to_table.binary_search(&pkp_s).is_ok() && // from is H1
               pkp_t != person_id_val && // to is not source
               distinct_hop1_to_table.binary_search(&pkp_t).is_err() && // to is not H1
        ...
        // Hop 3 friends (distinct_hop3_to_table) & ext
        let mut hop3_friends_raw = Vec::new();
        for edge in &person_knows_person_input {
            let pkp_s = edge[0];
            let pkp_t = edge[1];
            if distinct_hop2_to_table.binary_search(&pkp_s).is_ok() && // from is H2
               pkp_t != person_id_val && // to is not source
               distinct_hop1_to_table.binary_search(&pkp_t).is_err() && // to is not H1
               distinct_hop2_to_table.binary_search(&pkp_t).is_err() && // to is not H2
        ...
        // --- Hop 4 Additions (Pre-computation for H3_ext) START ---
        ...
        // --- Hop 4 Additions (Pre-computation for H3_ext) END ---

        // --- Hop 4 Additions (Pre-computation for distinct_hop4_to_table) START ---
        let mut hop4_friends_raw = Vec::new();
        for edge in &person_knows_person_input {
            let pkp_s = edge[0];
            let pkp_t = edge[1];
            if distinct_hop3_to_table.binary_search(&pkp_s).is_ok() && // from is H3
               pkp_t != person_id_val && // to is not source
               distinct_hop1_to_table.binary_search(&pkp_t).is_err() && // to is not H1
               distinct_hop2_to_table.binary_search(&pkp_t).is_err() && // to is not H2
               distinct_hop3_to_table.binary_search(&pkp_t).is_err() && // to is not H3
        ...
        // --- Hop 4 Additions (Pre-computation for distinct_hop4_to_table) END ---
        ...
        layouter.assign_region(
            ...
            |mut region| {
                ...
                for (i, edge) in ordered_pkp_table.iter().enumerate() {
                    ...
                    // --- Hop 4 Additions (PKP Loop Assignments for H3 Align & Flag) START ---
                    ...
                    // --- Hop 4 Additions (PKP Loop Assignments for H3 Align & Flag) END ---
                }

                // Helper function for find_aligned_pair (used above)
                fn find_aligned_pair<F: Field + Ord>(ext_values: &[F], val_to_search: F) -> (F, F) {
                        ...
                        .binary_search_by(|p| f_to_u64(p).cmp(&f_to_u64(&val_to_search)))
                    {
                        ...
                        Err(idx) => {
                            ...
                            // If idx == 0, aligned_val remains F::ZERO
                            // If idx == ext_values.len(), next_aligned_val remains MAX_PERSON_ID
                        }
                    }
                    (aligned_val, next_aligned_val)
                }

                // Assign distinct_hop1_to table & ext
                ...
                // Assign distinct_hop2_to table & ext
                ...
                // Assign distinct_hop3_to table & ext
                ...
                // --- Hop 4 Additions (Assign H3_ext tables) START ---
                ...
                // --- Hop 4 Additions (Assign H3_ext tables) END ---

                // --- Hop 4 Additions (Assign distinct_hop4_to table) START ---
                ...
                // --- Hop 4 Additions (Assign distinct_hop4_to table) END ---
...
mod tests {
    use super::*;
    mod utils {
        /* Minimal stubs from previous response */
