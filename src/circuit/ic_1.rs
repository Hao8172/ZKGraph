...
pub struct ic1CircuitConfig<F: Field + Ord> {
    ...
    pkp_norm_order_config: LtEqGenericConfig<F, NUM_BYTES>, // 验证 pkp_norm_0 <= pkp_norm_1

    pc_norm_0: Column<Advice>, // min(predecessor, person[0])
    pc_norm_1: Column<Advice>, // max(predecessor, person[0])
    pc_norm_order_config: LtEqGenericConfig<F, NUM_BYTES>, // 验证 pc_norm_0 <= pc_norm_1
    ...
    // for 多对多 `(current_val_in_top20_ext, next_val_in_top20_ext)`
    ...
    // email
    person_email_emailaddress: Vec<Column<Advice>>, // A
    ordered_person_email_emailaddress: Vec<Column<Advice>>, // A
    ordered_person_email_configure: LtEqGenericConfig<F, NUM_BYTES>,
    q_ordered_email: Selector,
    aligned_email_personid: Column<Advice>,      // D
    next_aligned_email_personid: Column<Advice>, // D 辅助列
    q_email: Selector,
...
impl<F: Field + Ord> ic1Chip<F> {
    ...
    pub fn configure(meta: &mut ConstraintSystem<F>) -> ic1CircuitConfig<F> {
        ...
        // 构造 IsZeroChip，验证 person[0] 与 person_id 是否相等
        ...
        // 3. 点dist == 前驱点dist + 1
        // 之所以 * (dist-4)，因为将dummy和source的前驱都设置成 0
        ...
        // 配置 LtEqGenericChip 用于验证 pkp_norm_0 <= pkp_norm_1
        // LtEqGenericChip 的 is_lt(lhs, rhs) 检查 lhs < rhs
        // 我们希望 pkp_norm_1 < pkp_norm_0 为 false (0)
        let pkp_norm_order_config = LtEqGenericChip::configure(
            meta,
            |meta| meta.query_selector(q_edge_exists[1]), // 激活时机：当处理 person_knows_person 表的行时
        ...
        meta.create_gate("pkp_normalization_check", |meta| {
            ...
            // 1. 验证顺序: norm_1 < norm_0 应该为 false (0)
            let order_check = pkp_norm_order_config.is_lt(meta, None);

            // 2. 验证元素集合是否相同: (norm_0 + norm_1) == (u + v) AND (norm_0 * norm_1) == (u * v)
            ...
            vec![
                q.clone() * sum_check,
                q.clone() * prod_check,
                q * order_check, // 这个值应该是0
            ]
        });

        // 配置 LtEqGenericChip 用于验证 pc_norm_0 <= pc_norm_1
        // 我们希望 pc_norm_1 < pc_norm_0 为 false (0)
        let pc_norm_order_config = LtEqGenericChip::configure(
            meta,
            |meta| meta.query_selector(q_edge_exists[0]), // 激活时机：当处理 person 表中需要验证边的行时
        ...
        meta.create_gate("pc_normalization_check", |meta| {
            ...
            let c = meta.query_advice(person[0], Rotation::cur()); // person[0] 是当前节点ID
        ...
        // 4. 修改后的边查找：(norm_P, norm_C) --lookup--> (norm_U, norm_V)
        ...
        // 6. 边[0] + source点dist --lookup--> 点+点dist
        ...
        // 7. 边[1] + target 点dist --lookup--> 点+点dist
        ...
        // align[i]必须是top20_ext中的一个，lookup
        ...
        // align[i] <= ordered_person_email_emailaddress[0] 即 personid
        ...
        // next_align_email_larger_chip <= next_align[i] - 1 即 personid
        ...
        // align[i] <= ordered_person_speaks_language[0] 即 personid
        ...
        // next_align_language_larger_chip <= next_align[i] - 1 即 personid
        ...
        // align[i] <= ordered_person_studyAt_organisation[0] 即 personid
        ...
        // next_align_university_larger_chip <= next_align[i] - 1 即 personid
        ...
        // 添加 shuffle 约束来连接 ordered_person_studyAt_organisation 和 top_universities
        ...
        meta.shuffle(format!("company top shuffle"), |meta| {
            ...
            let q2 = meta.query_selector(q_ordered_com_flag); // 复用q_ordered_flag选择器
        ...
        meta.lookup_any(format!("university organisation lookup"), |meta| {
            ...
            let a = meta.query_advice(top_universities[1], Rotation::cur()); // 组织ID
            let b = meta.query_advice(top_universities[3], Rotation::cur()); // 组织名称
            let c = meta.query_advice(organisation[0], Rotation::cur()); // 组织ID
            let d = meta.query_advice(organisation[2], Rotation::cur()); // 组织名称
            let lhs = [one.clone(), a, b].map(|c| c * q1.clone());
            let rhs = [one.clone(), c, d].map(|c| c * q2.clone());
            lhs.into_iter().zip(rhs).collect()
        });
        ...
        meta.lookup_any(format!("company organisation lookup"), |meta| {
            let q1 = meta.query_selector(q_top_companies);
            let q2 = meta.query_selector(q_organisation);
            let a = meta.query_advice(top_companies[1], Rotation::cur()); // 组织ID
            let b = meta.query_advice(top_companies[3], Rotation::cur()); // 组织名称
            let c = meta.query_advice(organisation[0], Rotation::cur()); // 组织ID
            let d = meta.query_advice(organisation[2], Rotation::cur()); // 组织名称
            let lhs = [one.clone(), a, b].map(|c| c * q1.clone());
            let rhs = [one.clone(), c, d].map(|c| c * q2.clone());
        ...
        meta.lookup_any(format!("university place lookup"), |meta| {
            let q1 = meta.query_selector(q_top_universities);
            let q2 = meta.query_selector(q_place);
            let a = meta.query_advice(top_universities[4], Rotation::cur()); // 大学所在地ID
            let b = meta.query_advice(top_universities[5], Rotation::cur()); // 大学所在地名称
            let c = meta.query_advice(place[0], Rotation::cur()); // 地点ID
            let d = meta.query_advice(place[1], Rotation::cur()); // 地点名称
            let lhs = [one.clone(), a, b].map(|c| c * q1.clone());
            let rhs = [one.clone(), c, d].map(|c| c * q2.clone());
            lhs.into_iter().zip(rhs).collect()
        });
        meta.lookup_any(format!("company place lookup"), |meta| {
            let q1 = meta.query_selector(q_top_companies);
            let q2 = meta.query_selector(q_place);
            let a = meta.query_advice(top_companies[4], Rotation::cur()); // 公司所在地ID
            let b = meta.query_advice(top_companies[5], Rotation::cur()); // 公司所在地名称
            let c = meta.query_advice(place[0], Rotation::cur()); // 地点ID
            let d = meta.query_advice(place[1], Rotation::cur()); // 地点名称
            let lhs = [one.clone(), a, b].map(|c| c * q1.clone());
            let rhs = [one.clone(), c, d].map(|c| c * q2.clone());
        ...
        meta.lookup_any(format!("university location lookup"), |meta| {
            ...
            let a = meta.query_advice(top_universities[1], Rotation::cur()); // 组织ID
            let b = meta.query_advice(top_universities[4], Rotation::cur()); // 地点ID
            let c = meta.query_advice(organisation_isLocatedIn_place[0], Rotation::cur()); // 组织ID
            let d = meta.query_advice(organisation_isLocatedIn_place[1], Rotation::cur()); // 地点ID
            let lhs = [one.clone(), a, b].map(|c| c * q1.clone());
            let rhs = [one.clone(), c, d].map(|c| c * q2.clone());
            lhs.into_iter().zip(rhs).collect()
        });
        meta.lookup_any(format!("company location lookup"), |meta| {
            ...
            let q2 = meta.query_selector(q_org_located);
            let a = meta.query_advice(top_companies[1], Rotation::cur()); // 组织ID
            let b = meta.query_advice(top_companies[4], Rotation::cur()); // 地点ID
            let c = meta.query_advice(organisation_isLocatedIn_place[0], Rotation::cur()); // 组织ID
            let d = meta.query_advice(organisation_isLocatedIn_place[1], Rotation::cur()); // 地点ID
            let lhs = [one.clone(), a, b].map(|c| c * q1.clone());
            let rhs = [one.clone(), c, d].map(|c| c * q2.clone());
        ...
        let top20_order_less_dist = LtEqGenericChip::configure(
            ...
            |meta| vec![meta.query_advice(top20_friends[10], Rotation::cur())], // 当前行距离
            |meta| vec![meta.query_advice(top20_friends[10], Rotation::next())], // 下一行距离
        );
        ...
        let top20_order_equal_dist_less_lastname = LtEqGenericChip::configure(
            meta,
            |meta| meta.query_selector(q_top20_order),
            |meta| vec![meta.query_advice(top20_friends[2], Rotation::cur())], // 当前行姓氏
            |meta| vec![meta.query_advice(top20_friends[2], Rotation::next())], // 下一行姓氏
        );
        ...
        let top20_order_equal_dist_equal_lastname_less_id = LtEqGenericChip::configure(
            meta,
            |meta| meta.query_selector(q_top20_order),
            |meta| vec![meta.query_advice(top20_friends[0], Rotation::cur())], // 当前行ID
            |meta| vec![meta.query_advice(top20_friends[0], Rotation::next())], // 下一行ID
        );
        ...
        // 创建门来验证排序是否正确
        meta.create_gate("验证TOP20朋友排序", |meta| {
        ...
        // 配置比较候选记录和最后一个top20记录
        let candidate_vs_last_dist = LtEqGenericChip::configure(
            meta,
            |meta| meta.query_selector(q_candidate_check),
            |meta| vec![meta.query_advice(top20_friends[10], Rotation(19))], // 最后一个top20记录的距离
            |meta| vec![meta.query_advice(candidate_record[2], Rotation::cur())], // 候选距离
        );

        let candidate_vs_last_lastname = LtEqGenericChip::configure(
            meta,
            |meta| meta.query_selector(q_candidate_check),
            |meta| vec![meta.query_advice(top20_friends[2], Rotation(19))], // 最后一个top20记录的姓氏
            |meta| vec![meta.query_advice(candidate_record[1], Rotation::cur())], // 候选姓氏
        );

        let candidate_vs_last_id = LtEqGenericChip::configure(
            meta,
            |meta| meta.query_selector(q_candidate_check),
            |meta| vec![meta.query_advice(top20_friends[0], Rotation(19))], // 最后一个top20记录的ID
            |meta| vec![meta.query_advice(candidate_record[0], Rotation::cur())], // 候选ID
        );
    ...
    pub fn assign(
        ...
        fn generate_align(orign: &Vec<u64>, top20_extension: &Vec<u64>) -> Vec<u64> {
            ...
            for &a_val in orign {
                // 找到小于等于a_val的最大b值
                let mut max_b = top20_extension[0]; // 默认为最小值
        ...
        let mut distances = HashMap::new(); // 存储节点到源点的距离
        let mut predecessors = HashMap::new(); // 存储每个节点的前驱节点
        let mut q = VecDeque::new(); // BFS 队列

        // 初始化所有 person_table 中节点的距离为虚拟距离
        ...
        if distances.contains_key(&person_id_val) {
            distances.insert(person_id_val, 0); // 源点距离为0
            q.push_back(person_id_val); // 源点入队
                                        // 源点的前驱可以设为自身，距离为0，以简化约束处理
            predecessors.insert(person_id_val, person_id_val);
        } else {
            println!("警告: BFS 源 Person ID 未在 person_table 中找到。");
        }
        ...
        while let Some(u_id) = q.pop_front() {
            ...
            if let Some(neighbors) = adj.get(&u_id) {
                for &v_id in neighbors {
                    // 如果 v_id 未被访问过 (距离仍为虚拟距离)
                    if distances
                        .get(&v_id)
                        .map_or(false, |&d| d == dummy_distance_u64)
                    {
                        distances.insert(v_id, dist_u + 1); // 更新距离
                        predecessors.insert(v_id, u_id); // 设置前驱
                        q.push_back(v_id); // 邻居入队
                    }
                }
            }
        }
        ...
        // distances.iter()
        //     .filter(|(_, &v)| v == 1)  // 过滤出 value == 1 的条目
        //     .for_each(|(k, _)| println!("distance=1 Key: {}", k));

        // 找出满足条件的前20个朋友(按距离、lastName和ID排序)
        ...
        all_candidates.sort_by(|a, b| {
            a.2.cmp(&b.2) // 首先按距离排序
                .then_with(|| a.1.cmp(&b.1)) // 然后按lastName排序
                .then_with(|| a.0.cmp(&b.0)) // 最后按ID排序
        });
        ...
        for (i, row) in ordered_person_studyAt_organisation.iter().enumerate() {
            if university_flag[i] {
                ...
                if let Some(org) = organisation.iter().find(|org_row| org_row[0] == row[1]) {
                    university_row.push(org[2]); // 添加组织名称
                } else {
                    university_row.push(0); // 如果找不到，添加默认值
                }
        ...
        for (i, row) in ordered_person_workAt_organisation.iter().enumerate() {
            if company_flag[i] {
                ...
                // 查找对应的组织名称
                if let Some(org) = organisation.iter().find(|org_row| org_row[0] == row[1]) {
                    company_row.push(org[2]); // 添加组织名称
                } else {
                    company_row.push(0); // 如果找不到，添加默认值
                }
        ...
        for row in &organisation_isLocatedIn_place {
            org_to_place.insert(row[0], row[1]); // 组织ID -> 地点ID
        }
        ...
        for row in &top_universities_data {
            ...
            if let Some(&place_id) = org_to_place.get(&row[1]) {
                expanded_row.push(place_id); // 添加地点ID

                // 查找地点名称
                if let Some(place_info) = place.iter().find(|p| p[0] == place_id) {
                    expanded_row.push(place_info[1]); // 添加地点名称
                } else {
                    expanded_row.push(0); // 找不到地点信息，添加默认值
                }
            } else {
                expanded_row.push(0); // 没有地点信息，添加默认值
                expanded_row.push(0);
            }
        ...
        layouter.assign_region(
            || "witness assignment",
            |mut region| {
                // 为每个 person 行赋值，person_row 的排列为：
                for (i, row) in person_table.iter().enumerate() {
                    ...
                    let (predecessor_val, predecessor_dist) = if row[0] == person_id_val {
                        ...
                        // 可达节点 (非源点)：前驱是 BFS 找到的 predecessor，前驱距离是 dist - 1
                        let pred_id = predecessors.get(&row[0]).cloned().unwrap();
                        // if i == 70 {
                        //     println!("pre:{:?},cur:{:?}"pred_id,row[0]);
                        // }
                    ...
                    // 赋值 pc_norm_0, pc_norm_1 (P,C)
                    if row[0] != person_id_val && calculated_dist != dummy_distance_u64 {
                        self.config.q_edge_exists[0].enable(&mut region, i)?; // 激活 P,C 侧的规范化和查找

                        let p_val = predecessor_val;
                        let c_val = row[0]; // 当前节点 ID
                        ...
                        // 为 pc_norm_order_chip 赋值: 检查 norm_c_val < norm_p_val 是否为 false
                        pc_norm_order_chip
                            .assign(
                                &mut region,
                                i,
                ...
                for (i, edge) in person_knows_person.iter().enumerate() {
                    ...
                    // 为 pkp_norm_order_chip 赋值: 检查 norm_v_val < norm_u_val 是否为 false
                    pkp_norm_order_chip
                        .assign(
                            &mut region,
                            i,
                        ...
                        .enumerate()
                {
                    ...
                    // 为 ordered_person_email_emailaddress 赋值
                    region.assign_advice(
                        || format!("email person_id row {}", i),
                    ...
                    align_email_less_chip
                        ...
                        .unwrap();
                    // 为 next_aligned_email_personid 赋值
                    region.assign_advice(
                        ...
                        .enumerate()
                {
                    ...
                    // 为 ordered_person_speaks_language 赋值
                    region.assign_advice(
                    ...
                    // 为 aligned_language_personid 赋值
                    region.assign_advice(
                    ...
                    align_language_less_chip
                        ...
                        .unwrap();
                    // 为 next_aligned_language_personid 赋值
                    region.assign_advice(
                        ...
                        .enumerate()
                {
                    ...
                    // 为 ordered_person_studyAt_organisation 赋值
                    region.assign_advice(
                    ...
                    // 为 aligned_university_personid 赋值
                    region.assign_advice(
                    ...
                    align_university_less_chip
                        ...
                        .unwrap();
                    // 为 next_aligned_university_personid 赋值
                    region.assign_advice(
                ...
                if top20_candidates.len() == 20 {
                    let last_top20_idx = 19; // 最后一个top20元素的索引
...
mod tests {
    ...
    fn test_ic1_circuit() {
        ...
        let prover = MockProver::run(k, &circuit, vec![public_input]).expect("MockProver 运行失败");
