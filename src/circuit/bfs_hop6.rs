...
const MAX_PERSON_ID: u64 = 100000000000000; // Beispielwert, anpassen falls nötig
...
pub struct Is1CircuitConfig<F: Field + Ord + std::hash::Hash> {
    ...
    // --- Hop 0 & 1 States ---
    potential_friend_hop1: Column<Advice>, // Erreichbar in 0 oder 1 Hop
    dist0_node: Column<Advice>, // Ist Knoten die Quelle?
    dist1_node: Column<Advice>, // Ist Knoten 1-Hop von Quelle?
    q_dist1_node: Selector,

    source_pkp_zero: crate::chips::is_zero::IsZeroConfig<F>, // PKP Quelle == person_id?
    dist1_pkp_check: Column<Advice>, // Flag wenn PKP Quelle == person_id

    source_pkp_to_zero: crate::chips::is_zero::IsZeroConfig<F>, // PKP Ziel == person_id?
    source_pkp_to_check: Column<Advice>, // Flag wenn PKP Ziel == person_id

    // --- Distinct 1-Hop Friends ---
    q_hop1_to: Selector, // Selektor für Kanten, die zu 1-Hop Freunden führen
    ...
    // --- Extended Distinct 1-Hop Friends (für Alignment für Hop 2) ---
    ...
    distinct_hop1_to_ext_pairs_lookup_table: Vec<Column<Advice>>, // Eindeutig für Hop 1 ext
    q_distinct_hop1_to_ext_pairs_lookup: Selector, // Eindeutig für Hop 1 ext

    // --- Hop 1 Alignment (für PKP Kanten, um festzustellen, ob Quelle/Ziel ein 1-Hop Freund ist) ---
    ...
    h1_match_flag_config:crate::chips::is_zero::IsZeroConfig<F>, // Ist PKP Quelle ein 1-Hop Freund?
    ...
    h1_pkp_to_flag_config:crate::chips::is_zero::IsZeroConfig<F>, // Ist PKP Ziel ein 1-Hop Freund?
    h1_pkp_to_flag:Column<Advice>,
    
    // --- Hop 2 States & Distinct Friends ---
    potential_friend_hop2: Column<Advice>, // Erreichbar in 0, 1, oder 2 Hops
    dist2_node: Column<Advice>, // Ist Knoten 2-Hops von Quelle?
    q_dist2_node: Selector,
    q_hop2_to: Selector, // Selektor für Kanten, die zu 2-Hop Freunden führen
    ...
    // --- Extended Distinct 2-Hop Friends (für Alignment für Hop 3) ---
    ...
    distinct_hop2_to_ext_pairs_lookup_table: Vec<Column<Advice>>, // Eindeutig für Hop 2 ext
    q_distinct_hop2_to_ext_pairs_lookup: Selector, // Eindeutig für Hop 2 ext

    // --- Hop 2 Alignment (für PKP Kanten, um festzustellen, ob Quelle/Ziel ein 2-Hop Freund ist) ---
    ...
    h2_match_flag_config: crate::chips::is_zero::IsZeroConfig<F>, // Ist PKP Quelle ein 2-Hop Freund?
    ...
    h2_pkp_to_flag_config: crate::chips::is_zero::IsZeroConfig<F>, // Ist PKP Ziel ein 2-Hop Freund?
    h2_pkp_to_flag: Column<Advice>,
    
    // --- Hop 3 States & Distinct Friends ---
    potential_friend_hop3: Column<Advice>, // Erreichbar in 0, 1, 2, oder 3 Hops
    dist3_node: Column<Advice>, // Ist Knoten 3-Hops von Quelle?
    q_dist3_node: Selector,
    q_hop3_to: Selector, // Selektor für Kanten, die zu 3-Hop Freunden führen
    ...
    // --- Extended Distinct 3-Hop Friends (für Alignment für Hop 4) ---
    ...
    distinct_hop3_to_ext_pairs_lookup_table: Vec<Column<Advice>>, // Eindeutig für Hop 3 ext
    q_distinct_hop3_to_ext_pairs_lookup: Selector, // Eindeutig für Hop 3 ext

    // --- Hop 3 Alignment (für PKP Kanten, um festzustellen, ob Quelle/Ziel ein 3-Hop Freund ist) ---
    ...
    h3_match_flag_config: crate::chips::is_zero::IsZeroConfig<F>, // Ist PKP Quelle ein 3-Hop Freund?
    ...
    h3_pkp_to_flag_config: crate::chips::is_zero::IsZeroConfig<F>, // Ist PKP Ziel ein 3-Hop Freund?
    ...
    // --- Extended Distinct 4-Hop Friends (für Alignment für Hop 5) ---
    ...
    // --- Hop 4 Alignment (für PKP Kanten, Quelle/Ziel = 4-Hop Freund?) ---
    ...
    // --- Hop 6 Additions START ---
    // --- Extended Distinct 5-Hop Friends (für Alignment für Hop 6) ---
    ...
    // --- Hop 5 Alignment (für PKP Kanten, Quelle/Ziel = 5-Hop Freund?) ---
...
impl<F: Field + Ord + std::hash::Hash> Is1Chip<F> {
    ...
    pub fn configure(meta: &mut ConstraintSystem<F>) -> Is1CircuitConfig<F> {
        ...
        // ---------------------------
        // hop 1: Finde Freunde der Quellperson
        // ---------------------------
        ...
        // --- hop1_ext: Gepolsterte und sortierte Liste von 1-Hop-Freunden für Alignment-Lookups ---
        ...
        // --- Alignment-Spalten für pkp.FromId (ordered_pkp[0]) gegen distinct_hop1_to_ext ---
        ...
        // ---------------------------
        // hop 2: Finde Freunde von 1-Hop-Freunden
        // ---------------------------
        ...
        // --- hop2_ext: Gepolsterte und sortierte Liste von 2-Hop-Freunden für Alignment (für Hop 3) ---
        ...
        // --- Alignment-Spalten für pkp.FromId (ordered_pkp[0]) gegen distinct_hop2_to_ext ---
        ...
        // --- Hop 3: Finde Freunde von 2-Hop-Freunden ---
        ...
        // --- Hop 6 Additions (Configuration) START ---
        ...
        // --- Hop 6 Additions (Configuration) END ---
        ...
        Is1CircuitConfig {
            ...
            distinct_hop1_to_ext_pairs_lookup_table, // War vorher 'distinct_hop1_to_ext_pairs_lookup_table'
            q_distinct_hop1_to_ext_pairs_lookup,   // War vorher 'q_distinct_hop1_to_ext_pairs_lookup'
            ...
            distinct_hop2_to_ext_pairs_lookup_table, // Eindeutig benannt
            q_distinct_hop2_to_ext_pairs_lookup,   // Eindeutig benannt
            // Hop 2 Align
            ...
            distinct_hop3_to_ext_pairs_lookup_table, // Eindeutig
            q_distinct_hop3_to_ext_pairs_lookup,   // Eindeutig
    ...
    pub fn assign(
        ...
        // Alle Chips aus der Konfiguration erstellen
        ...
        // --- Hop 6 Additions (Chip Construction) START ---
        ...
        // Für Hop 6 wird keine `distinct_hop6_to_ext` benötigt, da es der letzte Hop ist.
        // --- Hop 6 Additions (Chip Construction) END ---


        // Lookup-Tabellen für LtEqGenericChips laden
        ...
        // --- Hop 6 Additions (Load LtEq Chips) START ---
        distinct_hop6_to_order_chip.load(layouter).unwrap();
        // --- Hop 6 Additions (Load LtEq Chips) END ---


        // BFS und Datenvorverarbeitung
        let max_hops = 6; // ERWEITERT für 6 Hops
        ...
        // Hop 1 Freunde (distinct_hop1_to_table)
        let mut hop1_friends_vec = Vec::new();
        for edge in &person_knows_person_input { // Iteriere über ursprüngliche PKP für korrekte Kanten von der Quelle
            if edge[0] == person_id_val {
                if edge[1] != person_id_val { // Schließe Quelle selbst aus
        ...
        // Hop 2 Freunde (distinct_hop2_to_table)
        ...
        // Hop 3 Freunde (distinct_hop3_to_table)
        ...
        // --- Hop 6 Additions (Pre-computation) START ---
        ...
        // --- Hop 6 Additions (Pre-computation) END ---

        layouter.assign_region(
            || "witness assignment",
            |mut region| {
                // Personentabelle und initiale Distanzzustände zuweisen
                for (i, p_row) in person_table.iter().enumerate() {
                    ...
                    let is_dist2 = dist == 2;
                    let is_dist3 = dist == 3;
                    let is_dist4 = dist == 4; // Hinzugefügt für Hop 4
                    let is_dist5 = dist == 5; // Hinzugefügt für Hop 5
                    let is_dist6 = dist == 6; // Hinzugefügt für Hop 6
                    ...
                    let dist4_val = F::from(is_dist4 as u64); // Hinzugefügt für Hop 4
                    let dist5_val = F::from(is_dist5 as u64); // Hinzugefügt für Hop 5
                    let dist6_val = F::from(is_dist6 as u64); // Hinzugefügt für Hop 6
                    ...
                    region.assign_advice(|| format!("dist4_node at {}", i), self.config.dist4_node, i, || Value::known(dist4_val))?; // Hinzugefügt für Hop 4
                    region.assign_advice(|| format!("dist5_node at {}", i), self.config.dist5_node, i, || Value::known(dist5_val))?; // Hinzugefügt für Hop 5
                    region.assign_advice(|| format!("dist6_node at {}", i), self.config.dist6_node, i, || Value::known(dist6_val))?; // Hinzugefügt für Hop 6
                    ...
                    if is_dist4 { self.config.q_dist4_node.enable(&mut region, i)?; } // Hinzugefügt für Hop 4
                    if is_dist5 { self.config.q_dist5_node.enable(&mut region, i)?; } // Hinzugefügt für Hop 5
                    if is_dist6 { self.config.q_dist6_node.enable(&mut region, i)?; } // Hinzugefügt für Hop 6
                    ...
                    let pot_friend_h4_val = if is_dist0 || is_dist1 || is_dist2 || is_dist3 || is_dist4 { F::ONE } else { F::ZERO }; // Hinzugefügt für Hop 4
                    let pot_friend_h5_val = if is_dist0 || is_dist1 || is_dist2 || is_dist3 || is_dist4 || is_dist5 { F::ONE } else { F::ZERO }; // Hinzugefügt für Hop 5
                    let pot_friend_h6_val = if is_dist0 || is_dist1 || is_dist2 || is_dist3 || is_dist4 || is_dist5 || is_dist6 { F::ONE } else { F::ZERO }; // Hinzugefügt für Hop 6
                    ...
                    region.assign_advice(|| format!("potential_friend_hop4 at {}", i), self.config.potential_friend_hop4, i, || Value::known(pot_friend_h4_val))?; // Hinzugefügt für Hop 4
                    region.assign_advice(|| format!("potential_friend_hop5 at {}", i), self.config.potential_friend_hop5, i, || Value::known(pot_friend_h5_val))?; // Hinzugefügt für Hop 5
                    region.assign_advice(|| format!("potential_friend_hop6 at {}", i), self.config.potential_friend_hop6, i, || Value::known(pot_friend_h6_val))?; // Hinzugefügt für Hop 6
                }

                // ordered_pkp Tabelle und zugehörige Flags zuweisen
                for (i, ordered_edge) in ordered_pkp_table.iter().enumerate() {
                    ...
                    // --- H4 Alignment Zuweisungen START ---
                    ...
                    // --- H4 Alignment Zuweisungen END ---

                    // --- H5 Alignment Zuweisungen START ---
                    ...
                    // --- H5 Alignment Zuweisungen END ---


                    // q_hopX_to Selektoren aktivieren
                ...
                // Authoritative Zuweisung für distinct_hop1_to
                ...
                // Authoritative Zuweisung für distinct_hop1_to_ext
                ...
                // Authoritative Zuweisung für distinct_hop2_to
                ...
                // Authoritative Zuweisung für distinct_hop2_to_ext
                ...
                // Authoritative Zuweisung für distinct_hop3_to
                ...
                // Authoritative Zuweisung für distinct_hop3_to_ext
                ...
                // --- Hop 4 Table Assignments END ---

                // --- Hop 5 Table Assignments START ---
                ...
                // --- Hop 5 Table Assignments END ---

                // --- Hop 6 Table Assignments START ---
                ...
                // Für Hop 6 gibt es keine `_ext` Tabelle, da es der letzte Hop ist.
                // --- Hop 6 Table Assignments END ---
...
mod tests {
    use super::*;
    mod utils {
        // Minimalstubs, ersetzen Sie diese durch Ihre tatsächlichen Utility-Funktionen
        use halo2_proofs::halo2curves::bn256::Fr;
        pub fn read_csv(path: &str, delimiter: char) -> Result<Vec<Vec<String>>, csv::Error> {
            if !std::path::Path::new(path).exists() {
                 // Erstellen Sie eine leere Datei oder eine Dummy-Datei, wenn sie nicht existiert, um den Testlauf zu ermöglichen
                // Für echte Tests muss der Pfad korrekt sein.
                if path.contains("person_0_0.csv") {
                     println!("Warnung: Person CSV nicht gefunden unter {}, verwende Dummy-Daten.", path);
                     return Ok(vec![vec!["0".to_string(),"a".to_string(),"b".to_string(),"male".to_string(),"2000-01-01".to_string(),"2000-01-01T00:00:00".to_string(),"0.0.0.0".to_string(),"browser".to_string()]]);
                } else if path.contains("person_knows_person_0_0.csv") {
                     println!("Warnung: PKP CSV nicht gefunden unter {}, verwende Dummy-Daten.", path);
                     return Ok(Vec::new()); // Leere Relationen
                }
            }
            let mut rdr = csv::ReaderBuilder::new().delimiter(delimiter as u8).from_path(path)?;
        ...
        pub fn string_to_u64(s: &str) -> u64 { s.chars().map(|c| c as u64).sum() } // Etwas besserer Stub
        pub fn parse_date(s: &str) -> u64 { s.len() as u64 } 
        pub fn parse_datetime(s: &str) -> u64 { s.len() as u64 } 
        pub fn ipv4_to_u64(s: &str) -> u64 { s.len() as u64 } 
    }
    use utils::{read_csv, string_to_u64, parse_date, parse_datetime, ipv4_to_u64};
    ...
    #[test]
    fn test_is1_circuit_6hop() { // Umbenannter Test
        let k = 14; // K ERHÖHT. Muss möglicherweise weiter angepasst werden.

        // HINWEIS: Geben Sie die tatsächlichen Pfade zu Ihren CSV-Dateien an
        // Für diesen Test verwende ich Platzhalterpfade, die zu Dummy-Daten führen, falls nicht vorhanden.
        let person_csv_path = "/home/wh/zkgraph/src/data/sf0.1/social_network/dynamic/person_0_0.csv";
        let pkp_csv_path = "/home/wh/zkgraph/src/data/sf0.1/social_network/dynamic/person_knows_person_0_0.csv";


        let person_data = read_csv(person_csv_path, '|').expect("Personen-Daten konnten nicht geladen werden");
        let relation_data = read_csv(pkp_csv_path, '|').expect("Relations-Daten konnten nicht geladen werden");

        let mut person_table: Vec<Vec<Fr>> = Vec::new();
        for row in person_data.iter() { 
            if row.len() < 8 { continue; } // Überspringe ungültige Zeilen
            let person_row = vec![
                Fr::from(row[0].parse::<u64>().unwrap_or(0)), // Fallback für Parse-Fehler
        ...
        // Stellen Sie sicher, dass die Personentabelle nicht leer ist, fügen Sie ggf. einen Dummy-Eintrag hinzu
        if person_table.is_empty() {
            println!("Warnung: Personentabelle ist leer nach dem Laden, füge Dummy-Person hinzu.");
            person_table.push(vec![Fr::from(21990232556585u64); 8]); // Dummy-Person mit der Test-ID
        }


        let mut person_knows_person: Vec<Vec<Fr>> = Vec::new();
        for row in relation_data.iter() { 
            if row.len() < 2 { continue; } // Überspringe ungültige Zeilen
            let r_row = vec![
                Fr::from(row[0].parse::<u64>().unwrap_or(0)),
                Fr::from(row[1].parse::<u64>().unwrap_or(0)),
            ];
        ...
        // Stellen Sie sicher, dass die Test-ID in der Personentabelle vorhanden ist
        if !person_table.iter().any(|p| p[0] == person_id_fr) {
             println!("Warnung: Test-Personen-ID {} nicht in Personentabelle gefunden. Füge sie hinzu.", test_person_id_val);
             let mut dummy_person_row = vec![Fr::from(string_to_u64("DummyFirstName")), Fr::from(string_to_u64("DummyLastName")), Fr::from(0u64), Fr::from(parse_date("1990-01-01")), Fr::from(parse_datetime("1990-01-01T00:00:00")), Fr::from(ipv4_to_u64("127.0.0.1")), Fr::from(string_to_u64("DummyBrowser"))];
             dummy_person_row.insert(0, person_id_fr);
             person_table.push(dummy_person_row);
        }
        ...
        let public_input = vec![Fr::from(1)]; // Beispielhafter öffentlicher Input
        let start = Instant::now();
        let prover = MockProver::run(k, &circuit, vec![public_input.clone()]).expect("MockProver konnte nicht ausgeführt werden");
        println!("Prover Ausführungszeit: {:?}", start.elapsed());

        match prover.verify() {
            Ok(_) => println!("Verifizierung erfolgreich!"),
            Err(e) => {
                for error in e.iter() {
                    println!("{:?}", error);
                }
                panic!("Verifizierung fehlgeschlagen, siehe Details oben.");
            }
        }
    }
}