...
fn main() {
    ...
    let post_data = read_csv(
        "/home/wh/zkgraph/src/data/message_fact/180k/post_hasCreator_person.csv",
        '|',
    )
    ...
    let comment_data = read_csv(
        "/home/wh/zkgraph/src/data/message_fact/180k/comment_hasCreator_person.csv",
        '|',
    )
    .expect("Failed to read comment data");

    let person_data = read_csv(
        "/home/wh/zkgraph/src/data/message_fact/180k/person_0_0.csv",
        '|',
    )
    .expect("Failed to read person data");
    println!(
        "post_data.len(): {}, comment_data.len(): {}, person_data.len(): {}",
        post_data.len(),
        comment_data.len(),
        person_data.len()
    );
