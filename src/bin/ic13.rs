...
fn main() {
    ...
    let person_data = read_csv(
        "/home/wh/zkgraph/src/data/person_fact/180k/person_0_0.csv",
        '|',
    )
    .expect("无法加载 Person 数据");
    let relation_data = read_csv(
        "/home/wh/zkgraph/src/data/person_fact/180k/person_knows_person_0_0.csv",
        '|',
    )
    ...
    let circuit = MyCircuit::<Fr> {
        person: person_table,
        person_knows_person,
        person_id1: Fr::from(17592186055119),
        person_id2: Fr::from(8796093025131),
        _marker: PhantomData,
    };
